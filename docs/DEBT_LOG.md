...
- 후속: 없애려면 `LVS_OWNERDRAWFIXED` + `WM_MEASUREITEM`으로 가야 하고 **모든 열을 직접 그려야 한다**(표 6곳 영향). 스페이서만 빼면 체크박스 없는 표 4곳의 행 높이가 무너지고, 체크박스 표에서만 빼면 `OnCreate`↔`SetColumns` 호출 순서에 의존하게 된다. **첫 열 아이콘 요구가 생길 때 함께 전환한다**
