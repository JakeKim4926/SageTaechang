...
### [2026-08-07] 검증구멍 — 「시작 직후 화면」이 어느 확인 목록에도 없다
- 위치: 확인 절차 자체. 계기는 `SageTaechangView::CreateChildControls`의 회귀(아래 해결됨 항목)
- 설명: 4d-3에서 시작 시 라벨 적용이 빠졌는데 **4d-3도, D7-4 1단계도 「확인 완료」로 통과했다.** 두 확인 목록이 모두 「실행 · 완료 · 실패 · 초기화 · 드롭 · **워크플로 전환**」이었고, 전환을 한 번이라도 하면 증상이 사라지는 종류였기 때문이다. **첫 화면은 사용자가 매번 보는 화면인데 확인 항목에 없었다.**
- 위험도: 중 — 같은 성격(초기 상태에만 나타나고 조작하면 사라지는)의 결함을 계속 놓친다
- 후속: 화면을 건드리는 Step의 확인 목록 **첫 줄을 「앱을 새로 띄운 직후 화면」**으로 고정한다. D7-4 3단계 확인부터 적용한다
