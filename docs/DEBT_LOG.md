...
### [2026-08-07] 작업사고 — 일괄 치환 도구가 세 번째로 인코딩을 깨뜨렸다
- 위치: 작업 방식. 4d-3(`sed -i`, CRLF 1,683줄) · D7-4 3단계(`sed -i`, `ClearStatusCard` 무한 재귀 + CRLF) · D7-5 1단계(python heredoc, 한글이 깨져 치환 실패 + CRLF 939줄)
- 설명: 소스가 **UTF-8 BOM + CRLF**인데 `sed -i`와 python `io.open` 재작성이 둘 다 이것을 보존하지 않는다. 매번 복구했지만 **같은 실수가 세 번 반복**됐고, D7-4에서는 치환이 새 함수 본문까지 먹어 **무한 재귀**를 만들었다(빌드 전에 발견).
- 위험도: 중 — 조용히 깨지고, 커밋 뒤에 발견하면 diff 전체가 오염된다
- 후속: **한글이 든 파일이나 여러 줄에 걸친 치환은 Edit 도구로 한다.** `sed`/스크립트는 ASCII 한 줄 치환에만 쓰고, **쓴 직후 BOM·CRLF를 확인**한다. 함수 이름을 바꿀 때는 **정의부가 같이 걸리는지 먼저 본다**
