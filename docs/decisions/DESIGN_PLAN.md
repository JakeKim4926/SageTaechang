...
| D7-12 | 단가 계산 — 화면 구조 (목업 3-2) | **진행 중** 2026-08-20 등재 — D7-2가 미룬 배치. D7이 닫힐 때 함께 묻혔다 |
...
1. 콘텐츠 기준을 0,0으로 내리고 카드 3개 rect를 잡는다 → 검증: 좌우 여백이 다른 화면과 같다(24)
2. 카드 헤더 3개 (`CSageSectionLabel` 38px — 계산 조건 · 계산 결과 · 최근 생성 내역) → 검증: 세 밴드의 높이·글자 위치가 데이터 관리 화면과 같다
3. 좌우 2단 — 입력 420 고정 + 결과 가변, gap 20 → 검증: 두 카드가 한 줄에 있고 결과 카드가 남는 폭을 채운다
4. 운임 에딧을 입력 카드 3행으로 옮기고 결과 카드에 운임 값 라벨을 새로 만든다 → 검증: 운임을 고치면 결과의 운임 행과 합계가 함께 바뀐다
5. 버튼 2개를 입력 카드 안쪽 하단으로 — 「견적서 생성」 Primary(남는 폭) + 「초기화」 Ghost(72) → 검증: 아이콘 버튼이 화면에 남아 있지 않다
6. 결과 4행을 34px + 하단 1px `#EDE8E0`로 → 검증: 4줄 전부에 hairline이 있고 표 그리드선과 같은 색이다
7. 최근 생성 내역을 전체 폭 카드로 하고 품목명이 남는 폭을 흡수 → 검증: 카드 우측에 빈 공간이 없다
8. 라벨 정렬을 목업대로 (「페이지」만 우측, 나머지 좌측) → 검증: `DEBT_LOG` 정렬 항목이 닫힌다
