...
| 4 | 공백 행을 **프리젠터에서 제거** (목업 캡션·비교표 모두 명시) | — | **둘 다 아님 — 유지한다** | **고객사가 요청해서 넣은 행이다.** 목업 저자도 계획서도 이 사실을 몰랐다. 화면 표에서도 그대로 보여준다 (2026-08-06 사용자 확인) |
...
- **공백 행(`isSeparator`)은 그대로 표시한다.** 고객사 요청으로 들어간 행이다 — 어긋나는 지점 #4
