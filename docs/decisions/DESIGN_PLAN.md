...
| D7-12 | 단가 계산 — 화면 구조 (목업 3-2) | **완료** 2026-08-20 · 확인받음 — D7-2가 미룬 배치. 카드 헤더가 테두리를 덮던 결함을 **세 화면에서** 함께 고쳤다. 남은 목업 차이 5건은 해당 절 참조 |
...
| D9 | 모달 메시지 박스를 앱 스타일로 교체 (`AfxMessageBox` → `SageMessageBoxDlg`) | **완료** 2026-08-19 · 확인받음 · 1~4단계 전부 |
...
**Step 5(접두사 전환)와의 관계**: Step 5는 2026-08-20에 완료됐다. 이 계획에서 추가하는
상수도 **`SAGE_` 접두사로 만든다.** 이제 접두사는 하나뿐이므로 섞일 여지가 없다.
...
| R7 | `AfxMessageBox`가 11파일 **90곳**에 있다. 목업은 다이얼로그 검증을 인라인 라벨로 보여준다 | 전부 바꾸면 범위가 폭발한다 | **다이얼로그 입력 검증분만** 인라인으로 옮긴다. 파일 I/O 실패·DB 오류 등 화면 밖 사유는 `MessageBox` 유지. **수치 정정(2026-08-19): 현재 9파일 56곳이다**(D5a·D7-8 이관 후). 남은 모달은 없애는 것이 아니라 **앱 스타일로 바꾼다** — Step D9 |
...
> **2026-08-20 — 이 절이 예고한 「별도 Step」은 D7-12다.** 승격은 D7-4에서 끝났는데
> 이 화면에 배치되지 않아 헤더 밴드가 22px에 잘려 있었다. 이 표는 그때 실측한 4항목이고,
> D7-12 착수 전 대조에서 **10항목으로 늘었다.** 갱신된 목록은 D7-12를 본다.
...
### 나중에 드러난 것 — 다이얼로그 본문이 흰 면이어야 한다 (2026-08-19)

D7-9는 라벨 색 · 인라인 에러 · 버튼 위계만 다뤘고 **본문 배경을 보지 않았다.**
목업 3-9의 다이얼로그 컨테이너는 `background:#fff`이고 캡션 밴드만 `#F2EEE7`인데,
앱은 본문도 `APP_BACKGROUND`여서 **밴드와 본문이 같은 톤이라 캡션이 밴드로 읽히지 않았다.**
D9의 메시지 박스를 목업과 대조하다 사용자가 발견했다.

다이얼로그 8종의 배경 브러시를 `PANEL`로 바꾸고, 라벨 20개·인라인 에러 8개에 배경 역할을 지정했다.
`CSageInlineError`에는 `SetBackgroundRole`이 없어서 추가했다(기본값은 기존대로 `SAGE_BG_APP`이라 패널 사용처는 그대로다).

**교훈 25 — 색 분포를 셀 때 hover 값을 본문 값으로 읽지 않는다.**
목업에서 `#F7F6F1`이 3번 나와 본문 배경으로 읽었는데, 셋 다 `style-hover` 속성이었다.
**속성 이름까지 보고 세지 않으면 상태 값이 기본값으로 둔갑한다.**

### 목업 대조 (2026-08-19) — 6종 중 5종은 맞고 법인 선택만 어긋났다

D9 도중 흰 본문과 단가 추가 배치가 연달아 드러나서 **나머지 5종을 전수 대조했다.**

| 다이얼로그 | 결과 |
|---|---|
| 로그인 · 비밀번호 변경 | 구조 · 라벨 폭(64 / 96) · 문구 · 힌트까지 일치 |
| 법인 추가 · 법인 수정 | 라벨 위 + 입력 100%, 힌트, `SetCompanyContext`까지 일치 |
| **법인 선택** | 목록이 **Windows 기본 `CListBox`**였다 — 선택 행이 파란색이고 교대 행 · 행 구분선 · 행 높이가 전부 없었다 |

**사이드바 · 탭 · 표에서 걷어낸 파란 선택색이 이 화면 하나에 남아 있었다.**
`CSageListBox`를 신설해 목업 렌더링(선택 행 `#F7F2EA` + 카멜 SemiBold · 교대 행 `#FAF9F6` ·
하단 hairline `#EDE8E0` · 행 높이 32 · NC 1px 테두리)으로 맞췄다.

**교훈 26 — 「기본 컨트롤을 그대로 쓴 화면」은 색 점검에서 빠진다.**
D1a가 파란 선택색을 걷어낸 대상은 **커스텀 드로잉을 이미 하던 컨트롤**이었다.
`CListBox`는 손댄 적이 없어서 점검 목록에 오르지도 않았다. 색을 바꿀 때는
**바꾼 컨트롤이 아니라 같은 색을 쓸 수 있는 컨트롤 전체**를 세어야 한다.

### 남긴 차이 (고치지 않기로 한 것)

| 항목 | 목업 | 현재 | 사유 |
|---|---|---|---|
| 창 폭 | 전부 400 | 320 / 360 / 420 | **콘텐츠가 정한다.** 로그인은 두 줄이라 400이면 헐렁하다. 단가 추가에서 같은 이유로 360을 유지했다 |
| 버튼 폭 · 간격 | 내용 맞춤 · 8 | 96 고정 · 10 | 앱 전체가 96/10이다. 다이얼로그만 바꾸면 오히려 어긋난다 |
| 창 테두리 색 | `#C9BFB1` | 시스템 `WS_BORDER` | `SageFramelessDialog` 프레임 소관이라 **전 다이얼로그 공통 건**이다. 별 결정 |
| 법인 선택 검색 | 입력 + 🔍 통합 박스 | 에디트 단독 | `CSageSearchBox` 재사용 가능 여부를 먼저 본다. D7-6 등재 9번과 같은 항목 |
| 기준 칸 글자 정렬 | 좌측 (`padding:0 10px`) | **가운데** | **가운데를 유지한다** (2026-08-19 사용자 결정). 목업과 다른 것이 아니라 **사용자가 의도적으로 그렇게 둔 것**이다. 표 셀·헤더를 전부 가운데로 통일한 결정(D4b·D7)과 같은 방향이므로 되돌리지 않는다 |
| 기준 칸 글자 크기 | 12px | 14px | 앱 타입 스케일이 목업보다 한 단계 크다(R1 재결정 · D1b). 이 칸만 목업 값을 따르면 앱 안에서 갈라진다 |
| 표 필터의 세로 구분선 | 「선택 해제」와 「초기화」 사이 1px × 20 `#DCD6CD` | 없음 | **넣지 않는다** (2026-08-19 사용자 결정). 목업은 두 Ghost 버튼이 붙어 있어 그 사이를 가르지만, 우리는 「선택 해제」가 왼쪽 선택 바 안에 있어 구분선이 화면 절반을 가로지르게 된다. 목업 배치를 그대로 따르려면 선택 바 구조를 함께 바꿔야 하므로 별건이다 |
...
## D7-12. 단가 계산 — 화면 구조 (목업 3-2, 2026-08-20 등재)

대상: `SagePriceCalcPanel`

**D7-2는 표현 3개만 맞췄고 배치는 손대지 않았다.** 그 절의 *남은 목업 차이 — 화면 구조*가
「별도 Step 규모다 · 카드 헤더는 D7-5와 함께 본다」로 끝나 있는데, **D7-5 · D7-6이 닫히고
D7 전체가 닫힐 때 그 Step은 열리지 않았다.** 사용자가 화면과 목업을 대조해 지적했다(2026-08-20).

**교훈 — 「별도 Step 규모다」는 Step이 아니다.** 범위 밖으로 미룬 항목은 문서 안에 서술로 남으면
Step 목록에 없으므로 진행 상황 표에서 보이지 않고, 상위 Step이 닫히는 순간 함께 묻힌다.
D7-2 · D7-3의 *남은 목업 차이*는 둘 다 이 상태였다. **미루는 순간 번호를 붙인다.**

### 착수 전 대조 (2026-08-20, 코드 실측)

| # | 목업 3-2 | 현재 | 실측 근거 |
|---|---|---|---|
| 1 | 「계산 조건」(420 고정) + 「계산 결과」(가변) **좌우 2단**, gap 20 | **세로 스택** | `LayoutChildControls`가 `nY`를 누적한다. 결과 카드 폭은 `nResultPanelW = nInputPanelW`로 입력 카드에서 물려받는다 |
| 2 | 카드마다 38px 헤더 밴드 | 입력·결과 카드에 **제목이 없다** | `SAGE_UI_CALC_SECTION_RESULT`(「계산 결과」) **참조 0곳** |
| 3 | 최근 생성 내역도 카드(헤더 38px) | 섹션 라벨 22px + `WS_BORDER` 리스트 | `m_wndHistorySection`을 `SAGE_SECTION_TITLE_HEIGHT`(22)로 배치. **D7-4가 `CSageSectionLabel`을 38px 카드 헤더 규격으로 승격했으므로 밴드가 22px에 잘려 그려진다** — 목업 차이가 아니라 D7-4 여파 회귀 |
| 4 | 「견적서 생성」 Primary 넓게 + 「초기화」, 카드 **안쪽 하단** | 카드 **바깥 우측**에 32×32 아이콘 버튼 2개 세로 | `nBtnX = m_rectInputCard.right + SAGE_ROW_GAP`. 텍스트 라벨 `SAGE_UI_CALC_BTN`(「견적서 생성」)도 **참조 0곳** |
| 5 | 「운임」 입력은 계산 조건 카드 3행, 결과의 운임은 **읽기 전용 값** | 운임 에딧이 **결과 카드** 4행 | 입력과 결과가 한 카드에 섞여 있다. 결과 카드에 운임 값 라벨이 없다 |
| 6 | 결과 4행 34px + 각 행 하단 1px `#EDE8E0` | 행 44px, 구분선 **2개**(2px `SS_ETCHEDHORZ`) | `SAGE_EDIT_HEIGHT`(32) + `SAGE_CALC_RESULT_ROW_GAP`(12) |
| 7 | 카드 패딩 16 | 이 패널만 12 | `SAGE_CALC_PANEL_PADDING = 12` ↔ 다른 카드 `SAGE_CARD_PADDING = 16` |
| 8 | 콘텐츠 좌우 여백 24 | **40** | `SageWorkspacePanel::GetContentRect`가 이미 `SAGE_CONTENT_PAD_X`(24)를 뺀 rect를 주는데 패널이 `SAGE_MARGIN`(16)을 **또** 뺀다. `SageCompanyOrderPanel`은 0,0에서 시작한다 |
| 9 | 「부수 [120] **부**」 | 단위 없음 | 운임 행에는 「원」이 있는데 부수 행에는 없다 |
| 10 | 최근 생성 내역이 **전체 폭**, 품목명이 `1fr` | 카드 폭 = 컬럼 폭 합(고정 1,127) | 1440 창에서 우측에 49px가 남는다 |

### 결과 **완료** (2026-08-20, 빌드 + 화면 확인)

착수 전 대조 10항목을 전부 넣었다. 좌우 2단(입력 420 고정 + 결과 가변, gap 20) · 카드 헤더 3개 ·
운임 이동(입력 카드 3행 + 결과의 읽기 전용 값 행) · 버튼 2개를 입력 카드 안쪽 하단 텍스트 버튼으로 ·
결과 4행 34px + 하단 hairline · 최근 생성 내역 전체 폭 카드(품목명 stretch) · 콘텐츠 여백 40 → 24.
카드 높이는 상수 합으로 정한다 — 입력 **238**, 결과 **290**(테두리 2px 포함).

**교훈 — 「기존 화면과 같다」는 목업과 맞다는 근거가 아니다.** 착수 조사에서 헤더 밴드가 카드 좌·우·상단
테두리 1px을 덮는 것을 보고 *다른 패널도 같으니 일관적*이라며 넘겼다. 사용자가 화면을 보고
「카드들 다 헤더쪽에는 겉에 선이 없다」고 지적해 다시 보니, **세 화면이 똑같이 목업과 달랐던 것**이었다
(D7-4의 승격 때 심어졌다). 목업은 테두리가 헤더를 둘러싸고 밴드가 그 안쪽이다.
D7-11의 「스킬을 목업의 대체물로 썼다」와 같은 종류이며, 이번에는 **코드끼리 대조하고 목업을 열지 않은** 것이다.

밴드를 테두리 안쪽으로 1px 들여놓는 수정을 **세 화면에 함께** 넣었다(사용자 결정 — 한 화면만 고치면
카드 테두리가 화면마다 달라진다). 같은 산술이 `SageWorkflowInputPanel`에 **3곳 복제**돼 있어
`GetCardContentTop()`으로 모았다. 카드 높이 규격은 이제 세 화면 공통 `1 + 38 + 16 + 본문 + 16 + 1`이다.

### 남은 목업 차이 — 실측했고 아직 반영하지 않았다

| # | 목업 3-2 실측 | 현재 | 판단 |
|---|---|---|---|
| 1 | 입력·콤보·검색버튼 테두리 **`#C9BFB1`** (목업 3-2 안에 6곳) | `#DCD6CD`(카드 테두리와 같은 색) | **스킬 색표에 「입력 테두리」 역할이 없다.** 스킬 개정 + 패널·View 16곳 공통 결정이라 이 Step에서 정하지 않는다 |
| 2 | 결과 카드 콘텐츠 `padding:8px 16px 16px` | top 16 | 8은 4의 배수라 규격 위반이 아니다. 카드 패딩을 화면마다 갈라도 되는지가 결정 사항 |
| 3 | 표 셀은 숫자만, 결과 행만 `1,238,400 원`(공백) | 양쪽 다 `286,959원` | `SAGE_UI_CALC_WON_FORMAT` 하나를 표와 결과가 공유한다. 나누면 상수가 둘 |
| 4 | 표 「합계」 열 `font-weight:700` | 일반 | `SetHighlightColumns` 미지정. D4b가 「Bold는 목업이 굵은 열만」으로 정했고 3-2는 굵다 |
| 5 | 「초기화」에 `#C9BFB1` 테두리 (= Secondary) | Ghost(테두리 없음) | **스킬이 「초기화=Ghost」로 못 박은 지점.** 화면에서는 링크처럼 읽힌다. 스킬을 고칠지가 결정 사항 |

1·5는 스킬을 고쳐야 하고 3·4는 다른 화면의 표에 영향이 있다. **전부 이 Step 밖이며 누락이 아니다.**

**창 폭이 목업과 다르면 비율이 유지되지 않는다.** 목업은 1440 창에서 420:736(36:64)인데
1906 창에서는 420:1184(26:74)가 된다. 카드 폭을 캡하면(D7-6 선례) 아래 전체 폭 표와 어긋나므로
**stretch를 유지했다** — 창을 키우면 카드도 커지는 것이 이 앱의 기존 방식이고 표가 이미 그렇다.

### 목업에서 벗어나는 지점

**원칙: 폰트 크기 외에는 목업을 따른다.** 아래는 규격이 목업을 이기는 지점이고 전부 기존 결정이다.

| 목업 | 채택 | 사유 |
|---|---|---|
| 버튼 높이 36 | **34** (`SAGE_CARD_ACTION_BUTTON_HEIGHT`) | 스킬 *크기와 좌표* — 카드 안 액션 행만 34이고 이 버튼이 그 자리다 |
| 액션 행 위 간격 16 | **12** (`SAGE_CARD_ROW_GAP`) | `SageWorkflowInputPanel::GetInputCardHeight`가 이미 이 순서다. 카드 규격을 둘로 만들지 않는다 |
| 합계 값 20px / 800 | `SAGE_FONT_SUMMARY` 17px | 타입 스케일에 20px가 없다 (D7-2 결정 유지) |
| 표 헤더 34 · 행 34 | **규격 34** | 스킬 *표 렌더링 규격*. 목업 3-2도 34다 |
| `border-radius:6px` | 각진 사각 | R3 |
| 컬럼 폭 10종을 목업값으로 | **현재 폭 유지 + 품목명만 stretch** | 목업 고정 합은 818, 현재는 1,110이다. 폭 10개를 다시 배분하면 이번 배치 변경과 원인이 섞인다. 카드를 채우는 목적은 stretch 하나로 달성된다 |

### 함께 닫는 `DEBT_LOG` 항목

| 항목 | 처리 |
|---|---|
| [2026-08-07] 아이콘 버튼 2곳이 규격 상수를 쓰지 않는다 | **절반 해소** — 단가 계산 쪽은 텍스트 버튼이 되어 `SAGE_CALC_ICON_BTN_*` 3개가 orphan이 되므로 제거한다. 필터 초기화(`SageResultTablePanel`) 1곳은 남는다 |
| [2026-08-06] 폼 라벨 정렬이 화면마다 다르고 목업과도 어긋난다 | **단가 계산 쪽 해소** — 목업대로 「페이지」만 우측. 단가 관리 폼은 이번 범위 밖 |
| [2026-08-06] 단가 계산 입력 라벨에 6px 수동 보정 | **이미 사라져 있다** — `SAGE_CALC_INPUT_LABEL_SHIFT` · `SAGE_PRICE_COMPANY_LABEL_SHIFT` 둘 다 코드에 0곳이다. 항목이 낡았다 |

완료 기준: 「계산 조건」과 「계산 결과」가 **한 줄에 나란히** 있고 카드 3개에 38px 헤더 밴드가 있다.
운임 입력이 계산 조건 카드에 있고 결과 카드의 운임은 값만 보인다. 입력 카드 안쪽 하단에
텍스트 버튼 2개가 있고 화면에 아이콘 단독 버튼은 「법인 찾기」 하나뿐이다.

### 범위 밖

| 항목 | 사유 |
|---|---|
| 컬럼 폭 10종 재배분 | 위 *목업에서 벗어나는 지점* 참조. 배치와 폭을 한 커밋에 섞지 않는다 |
| `CSageEdit` 승격 (`DrawEditBorder` 제거) | 패널 16곳 공통 부채다. 이 화면만 바꾸면 테두리가 1px 이동해 다른 화면과 어긋난다 |
| 카드 틀을 `CSageCard`로 승격 | 카드 면·테두리를 부모가 `OnEraseBkgnd`에 그리는 것은 `SageCompanyOrderPanel` · `SageWorkflowInputPanel`과 같은 기존 방식이다. 승격은 3개 화면을 함께 봐야 한다 |
| 「초기화」 문자열 상수 4번째 추가 | `SAGE_UI_RESULT_RESET_BTN` · `SAGE_UI_INPUT_RESET_BTN`과 같은 값이 셋이 된다. 화면별 상수가 이 프로젝트 방식이므로 추가하되 **중복은 `DEBT_LOG`에 남긴다** |

---

# Step D9 — 모달 메시지 박스 교체 (2026-08-19 등재)
...
**「?」를 만들지 않는다** (2026-08-19 사용자 결정). `MB_ICONQUESTION`은 호출부 2곳뿐이고
...
**`MB_ICONQUESTION`은 경고 아이콘으로 매핑해 둔다.** 3단계 이후 호출부는 0곳이지만
표준 Win32 플래그라 매핑이 없으면 **아이콘 없는 창이 조용히 뜬다.** 세트를 3종으로 유지하면서
API를 온전하게 두는 쪽을 택했다 — 쓰이지 않는 일반화가 아니라 이미 존재하는 입력에 대한 정의다.

**완료 알림을 초록 체크로 만들지 않는다.** 완료 표현은 `CSageStatusCard`가 이미 갖고 있고,
모달까지 같은 표현을 쓰면 어느 쪽이 결과인지 갈린다. 모달은 「알림」이다.

### 플래그 없는 호출 8곳은 유형을 붙인다 (2026-08-19 사용자 결정)
...
- [x] **4단계** — 완료 (2026-08-19). `sagetaechang-ui`의 *다이얼로그* 절에 메시지 박스 규격과
      **본문 흰 면** 규격을 넣었고, `DEBT_LOG`의 「51곳」 항목을 정정했다.
      컨트롤 목록 표에는 넣지 않았다 — 그 표는 `app/ui/drawing/` 전용이고 이것은 다이얼로그다
...
| D9-2 | ~~워커 스레드에서 부르는 곳~~ | **해소 (2026-08-19)** — 워커는 `SageWorkflowController.cpp:126` 하나뿐이고 그 안에 모달이 0건이다. 완료 알림도 패널의 메시지 핸들러 안이라 **56곳 전부 UI 스레드**다 |
