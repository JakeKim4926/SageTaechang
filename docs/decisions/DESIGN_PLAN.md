...
| D4c | 반복 법인명 `〃` · 그룹 시작 행 SemiBold | 코드 완료 · **화면 확인 대기** |
...
다음 세션은 이 문서를 먼저 읽고 **D4c 화면 확인 → D5**로 이어간다.
...
| `CSageInlineError` | 변형 2종. **Error** = 배경 없음, 텍스트 `INLINE_ERROR_TEXT` 12px + 14px 아이콘(`ERROR` 색). **Warning** = 박스(`INLINE_WARN_BG` + `INLINE_WARN_BORDER`, padding 8/10, radius 4), 텍스트 `INLINE_WARN_TEXT` + 14px 아이콘(`WARNING` 색). 값이 비면 숨김 | 다이얼로그 6종 |
...
**인라인 에러는 위치가 폼 하단이 아니다.** 문제가 된 입력 **바로 아래**, 라벨 폭만큼
들여쓴다(목업 `padding-left:108px` = 라벨 96 + gap 12). 동시에 **그 에디트의 테두리가
`ERROR`로 바뀐다** (2026-08-06 사용자 결정). 어느 입력이 틀렸는지 화면에서 드러나야 한다.

**다이얼로그 높이가 늘어난다.** 캡처 「비밀번호변경」의 현재 DLG에는 에러 한 줄이 들어갈
세로 여유가 없다. 각 다이얼로그의 높이 재계산은 D7-9에서 한다.
