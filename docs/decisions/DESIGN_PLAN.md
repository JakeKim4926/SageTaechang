...
> **미검증 커밋 있음 (2026-08-07)** — D7-4 1단계(`35fb4e3`)는 **빌드·화면 확인을 받지 않은 채**
> `develop`에 머지됐다. 다음 작업을 얹기 전에 확인을 받는다.
> 특히 `CSageSectionLabel` 사용처 6곳이 동시에 바뀌므로 **결과 표 제목 · 실행 기록 ·
> 데이터 관리 2곳 · 단가 계산 기록**을 함께 봐야 한다.
...
| D7 | 화면별 적용 — 3장 9세트 | **진행 중** — D7-1 · D7-2 · D7-3 · D7-7 · D7-8 · D7-9 완료·확인받음. **3-B-4c~4d 완료로 막힘이 풀렸고 D7-4 1단계까지 진행됐다**(2·3단계 대기). 남은 것은 D7-4 2·3단계 · D7-5 · D7-6. 7-2·7-3은 **표현만** 맞췄고 화면 구조는 각 절의 *남은 목업 차이* 참조 |
...
| 단계 | 범위 | 상태 |
|---|---|---|
| 1 | 입력 카드 — 헤더 밴드 · 폼 라벨 80 · 버튼 우측 · 생성/초기화를 카드 안으로 | **완료** 2026-08-07 · **미검증** |
| 2 | 상태 카드 신설 — 대기/처리중/완료/실패 · 6px 진행바 · 「완료 · N건」 + 저장 경로 | 대기 |
| 3 | 「폴더 열기」 · 「결과 보기」 버튼 | 대기 |
