...
| 3 | 라벨 폭 **80 고정 (다이얼로그 포함)** | 로그인·단가 추가 **64px**, 비밀번호 변경 **96px** | **목업** | 「새 비밀번호 확인」8자는 80px에 안 들어간다. 목업이 96px를 쓴 이유다. 본문 화면만 80 고정, 다이얼로그는 64/96 |
...
| Step | 내용 | 상태 |
|---|---|---|
| D0 | `sagetaechang-ui` skill 개정 | 대기 |
| D1 | 토큰 상수 정의 · 서체 전환 (문서 우선순위 ①) | 대기 |
| D2 | 버튼 위계 4변형 (②) | 대기 |
| D3 | 컨트롤 높이 · 라벨 폭 통일 (③) | 대기 |
| D4 | 표 렌더링 (행 높이 · 격자 · 금액 · 공백 행) | 대기 |
...
| R9 | 라벨 폭을 줄이면 긴 라벨이 잘린다 (「현재 비밀번호」·「새 비밀번호 확인」) | 문구가 `...`로 잘린다 | 위 *어긋나는 지점* 3번대로 다이얼로그는 96px 허용. D3에서 라벨별로 `DrawText`+`DT_CALCRECT`로 실측 확인 |
...
**R1 해소 — 목업 px를 그대로 pt로 환산한다.** `CreatePointFont`는 0.1pt 단위이므로 ×10 값을 쓴다.

| 역할 | 목업 | pt (×10) | 현재 상수 | 현재 |
|---|---|---|---|---|
| 화면 제목 | 18px Bold | `135` | `TAECHANG_TITLE_FONT_POINT_SIZE` | 16pt |
| 섹션 제목 | 14px Bold | `105` | `TAECHANG_HEADER_FONT_POINT_SIZE` | 13pt |
| 본문 · 라벨 · 버튼 | 13px | `98` | `TAECHANG_CONTENT_FONT_POINT_SIZE` | 11pt |
| 표 셀 | 12px tabular | `90` | **신규** | — (본문 폰트 공유 중) |
| 캡션 · 상태바 | 11px | `83` | **신규** | — |

폰트 종류가 4종 → **6종**으로 늘어난다 (`SageUiResources.cpp:42-45`에 2개 추가).
표 셀·캡션이 본문과 분리되는 것이 문서 의도다.
...
| `TAECHANG_LABEL_WIDTH` | 90 | 80 |
| `TAECHANG_CALC_INPUT_LABEL_WIDTH` | 46 | 80 |
| `TAECHANG_CALC_RESULT_LABEL_WIDTH` | 92 | 80 |
| `TAECHANG_PRICE_FORM_LABEL_WIDTH` | 70 | 80 |
...
- [ ] C4 상수 9개 변경
- [ ] 높이 28→32 · 24→32에 따라 세로 좌표 재계산 (`LayoutChildControls` · 각 패널·다이얼로그 `LayoutControls`)
- [ ] `TAECHANG_EDIT_TEXT_TOP_PAD = 7`을 32px 기준으로 재조정
- [ ] `TAECHANG_BUTTON_TEXT_TOP_OFFSET = 2`는 Gmarket ascent 보정값이다. **Pretendard 기준으로 다시 재야 한다**
- [ ] 라벨별 `DT_CALCRECT` 실측으로 잘림 확인 (R9)
- [ ] 간격을 4의 배수로 정리

완료 기준
- 한 화면의 버튼·에디트·콤보 높이가 전부 32다
- 라벨이 `...`로 잘리는 곳이 없다
- 다이얼로그 6종의 라벨 폭이 64 또는 96이다
