...
| D5a | 인라인 에러 + 다이얼로그 7종 검증 이관 (④) | 진행 중 |
...
다음 세션은 이 문서를 먼저 읽고 **D5a**부터 이어간다.
...
| 요약 수치 | 16px Bold | `120` | **`128`** (17px) | — | **미도입** (D5) |
| 표 셀 | 12px tabular | `90` | **`98`** (13px) | `TAECHANG_LIST_FONT_POINT_SIZE` | 적용됨 |
| 캡션 · 상태바 | 11px · **12px** | `83` · `90` | **`90`** (12px) 하나로 합침 | — | **미도입** |
...
**다이얼로그 높이가 늘어난다.** 캡처 「비밀번호변경」의 현재 DLG에는 에러 한 줄이 들어갈
세로 여유가 없다. 에러 줄 자리를 **항상 비워 두고**(숨김 상태에서도 높이를 차지) 높이를
한 번만 키운다 — 나타날 때마다 다이얼로그가 커지면 버튼이 움직인다.
...
## D5a — `CSageInlineError` + 다이얼로그 검증 이관

- [ ] `CSageInlineError` (Error · Warning 2변형) + 14px 에러·경고 아이콘
- [ ] 다이얼로그 7종에 배치. 에러 줄 자리만큼 다이얼로그 높이를 키운다
- [ ] 입력 검증 `AfxMessageBox`를 인라인으로 이관 (R7 — **검증분만**)
- [ ] 에러 상태 에디트 테두리를 `ERROR`로
