...
D5a는 `feature/dialog-inline-error`에 있고 아직 develop에 머지하지 않았다.
다음 세션은 이 문서를 먼저 읽고 **D5b**부터 이어간다.
...
- [ ] C8 실측표대로 그린다 — 버튼 아이콘 15px·2px 기본, 추가 14, 드롭다운 12, 빈 표 22
- [ ] **D5가 먼저 쓰는 3종(빈 표 22 · 경고 14 · 에러 14)은 D5에서 만든다.** 아래 *Step 간 의존* 참조
