...
> **미검증 커밋 없음.** D7-4 1·2단계 모두 확인받았다. 브랜치 `feature/workflow-status-card`는
> 확인까지 끝났고 머지만 남았다(사용자 담당).
>
> **화면 Step의 확인 목록 첫 줄은 「앱을 새로 띄운 직후 화면」이다.** 2단계 확인에서
> 4d-3 회귀(시작 화면 라벨)가 이 항목으로 잡혔다. `DEBT_LOG` 검증구멍 항목 참조.

| 순서 | 할 일 | 근거 |
|---|---|---|
| ~~1~~ | ~~D7-4 2단계~~ | **완료** 2026-08-07 · 확인받음. `CSageStatusCard` 신설 + 4d-3 회귀 수정 |
| 1 | **D7-4 3단계** — 「폴더 열기」 · 「결과 보기」 | `DESIGN_PLAN` D7-4. **완료 카드 안 우측**에 30px Secondary 2개. 카드 높이 70이 이미 이 자리를 감안한 값이라 배치는 흔들리지 않는다 |
| 2 | **D7-5**(실행 기록 5컬럼 표) → **D7-6**(데이터 관리 좌우 2단 + ↑↓) | 대상이 패널이 됐다. D7-6은 착수 전 결정 2건이 남아 있다(좌우 2단 구조 · 카드마다 Primary) |
| 3 | **3-B-5a · 5b** (사이드바 · 헤더 패널) | View의 남은 워크플로 분기 9곳이 여기서 대부분 사라진다 |
