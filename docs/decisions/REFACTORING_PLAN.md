...
### 지금 어디인가 — **D7-1 · D7-7 · D7-8 완료, 전부 `develop`에 머지됨**

`develop` = `7886c78` (2026-08-06). **작업 브랜치는 남아 있지 않고 미검증 커밋도 없다.**
이날 세 작업이 들어갔다 — D7-1 2단계(합계 밴드) · D7-7·D7-8 1단계(선택 바) · D7-8 2단계(한 페이지 작성 박스).
각 결과와 교훈은 아래 *완료된 작업* 참조.

### 다음 세션은 여기서 고른다

아래 *앞으로의 순서* 표에서 **1번을 착수했다** (`fix/large-table-render`, 아래 Step 절).
끝나면 2번을 집는다. 3번(패널 분리)을 먼저 하면 2번의 일부(D7-4~D7-6)가 함께 풀린다.
착수 전에 `sagetaechang-plan`대로 Step을 이 문서에 적고,
**목업 대조는 색·폰트만이 아니라 세로 간격과 가장 긴 텍스트까지** 본다 (D7-1·D7-8 교훈).

## Step `fix/large-table-render` — 690행 표 렌더 (진행 중)

브랜치 `fix/large-table-render` · 규칙 출처 `coding-design`(컨트롤은 도메인을 모른다) ·
`coding-rules`(하드코딩 금지) · 측정치는 `DEBT_LOG` 2026-08-06 항목

**대상은 `SageResultTablePanel::RefreshRows`의 삽입 루프 한 곳이다.**
`DEBT_LOG` 후속 ①(`LPSTR_TEXTCALLBACK`)만 하고 ②(가상 리스트)는 하지 않는다.

### 착수 전 확인한 사실

콜백 전환이 성립하는 근거다. 「쓰이는 곳을 세어 보고 만든다」(D4c 교훈 16)를 따라 실제로 셌다.

| 확인 항목 | 결과 |
|---|---|
| `m_wndList`의 텍스트를 **외부**에서 읽는 코드 | **0곳.** `GetItemText`는 `SageListCtrl.cpp` 내부 5곳뿐이고 전부 **그리는 행에 한해** 돈다 (빈 금액 `—` 판정 · 1열 정렬 그리기 · `〃` 판정) |
| `SortItems` · `LVM_SORT` | **0곳.** 정렬이 없어 콜백 항목 재정렬 문제가 없다 |
| `SagePriceManagePanel`의 `GetItemText` 6곳 | **다른 리스트**(`m_wndCopiesList`)다. 이번 대상이 아니다 |
| 결과 필터 | `m_arrRows`를 직접 훑는다. 리스트 텍스트에 의존하지 않는다 |

### 작업 순서

- [ ] 삽입 루프를 `LPSTR_TEXTCALLBACK`으로 전환 — `SetItemText` 8회/행 제거, `LVIF_TEXT|LVIF_PARAM`을
      한 번의 삽입에 실어 `SetItemData` 호출도 함께 흡수한다
- [ ] 패널이 `LVN_GETDISPINFO`에 답한다 — `m_arrVisibleRows`와 `m_arrColumns`로 셀 문자열을 만든다.
      **`CSageListCtrl`에 행 데이터를 들려주지 않는다** (컨트롤은 도메인을 모른다)
- [ ] 사용자 빌드·화면 확인

### 완료 기준

- 690행 파일에서 **로드 · 워크플로 전환 · 필터 초기화의 멈춤이 눈에 띄게 짧아진다.**
  `SetItemText` 305ms가 사라지므로 580 → **약 270ms**가 예상값이다. 어긋나면 계측한다
- 표 내용이 이전과 **한 셀도 다르지 않다** — `〃` · 빈 금액 `—` · 그룹 시작 SemiBold ·
  체크박스 · 선택 바 건수가 그대로다

### 범위 밖

- **가상 리스트(`LVS_OWNERDATA`)** — `InsertItem` 221ms까지 없애지만 체크 상태를 우리가 보관해야 해서
  선택·복원 로직을 다시 짜야 한다. 별건이다
- **`InsertItem` 회당 320µs의 원인 규명** — 이전 세션에서 확정하지 못했고 이번 변경으로 사라지지 않는다

### 리스크

커스텀드로우 도중 `GetItemText`가 부모 통지(`LVN_GETDISPINFO`)를 유발하는 **재진입 구조**가 된다.
그리는 행에 한정되고 `〃` 판정은 앞 행도 읽어 행당 2회다. 화면이 깨지면 여기부터 본다.
...
| 순서 | 할 일 | 준비 상태 · 근거 |
|---|---|---|
| 1 | **`fix/large-table-render`** — 690행 표 렌더 580ms | **진행 중** (위 Step 절). 구간별 수치와 후속 2단계가 `DEBT_LOG`에 있다. 범위는 `RefreshRows` 한 곳 |
| 2 | **D7-2 · D7-3 · D7-9** | **막힘 없음** — 대상(`SagePriceCalcPanel` · `SagePriceManagePanel` · 다이얼로그 6종)이 이미 제자리에 있다 |
