...
### 지금 어디인가 — D7-1 **1단계 완료·검증됨**, 다음은 **2단계 합계 밴드**

브랜치 `feature/receivables-summary-bar`에 커밋 2개가 있고 **아직 `develop`에 머지하지 않았다.**

| 커밋 | 내용 | 검증 |
|---|---|---|
| `5dbc69e` | 미수금 결과 표 상단 요약 바 (D7-1 1단계) | 빌드 + 화면 확인 완료 |
| `c9c5697` | 워크플로 스크립트 7개의 Excel 프로세스 종료 | 생성 3회 연속 4초대 유지 확인 |

3-B-4a는 앞선 세션에 화면 9항목까지 확인을 마쳤다. **미검증 커밋은 없다.**

### 다음 작업 — D7-1 2단계 합계 밴드

`DESIGN_PLAN` D7-1의 남은 항목은 **합계 행**이다 (24건 · 4,655,610 · 273,000 · 4,382,610).
브랜치는 `feature/receivables-total-row`를 새로 딴다.

**이미 정해진 것 (다시 묻지 않는다 — 2026-08-06 사용자 결정)**
...
- `app/ui/drawing/SageTableTotalBar.h/.cpp` — 컬럼 폭 배열을 받아 셀 위치에 맞춰 금액을 우측 정렬.
  `CStatic` + `DrawItem` (`CSageSummaryBar`·`CSageEmptyState`와 같은 관용구)
- `vcxproj` · `filters` 등록 3단계를 빠뜨리지 않는다
- 패널의 `LayoutTableArea()`에 밴드 높이를 빼는 계산을 추가한다 (요약 바가 이미 같은 방식으로 들어가 있다)

**주의 — 컬럼 폭은 런타임에 바뀐다.** `UpdateColumnWidths()`가 `MulDiv`로 재배분하므로
밴드가 컬럼 폭을 캐시하면 창 크기 변경 시 어긋난다. 그릴 때마다 리스트에서 읽어야 한다.
