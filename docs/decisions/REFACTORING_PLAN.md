...
## 다음 세션 시작점 (2026-08-10)

### 다음 세션에 할 일 (2026-08-10 갱신)

> **미검증 커밋 없음. 디자인(D0~D7)과 구조 리팩토링(3-B)이 둘 다 닫혔다.**
...
| 1 | **입력 파일이 워크플로와 맞는지 검사한다** | 열린 부채 중 **유일하게 실사용 피해가 확인된 항목**이다 — 견적서 생성에 미수금 파일을 넣으면 오류 없이 엉뚱한 문서가 나온다(`DEBT_LOG` 2026-08-06, 위험도 중) |
| 2 | 남은 등재 항목 | 「기타 처리」 배지(등재 8 — `CSageBadge` 재사용이라 가장 싸다) · 검색 박스 좌측 칸(9) · 셸 높이 56(2b) · 셸 여백 24(2c) · 라운드 코너 R3(11) · 상태바(14) |
| 3 | 등재 13번(교대 행 그룹 단위) **생존 확인** | **D4c 철회(2026-08-09)로 그룹 개념 자체가 사라졌다.** 이 항목이 아직 유효한지부터 판정한다 — 무효면 근거를 적고 지운다 |
