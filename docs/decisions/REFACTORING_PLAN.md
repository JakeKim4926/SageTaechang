...
### 지금 어디인가 — D7-7 · D7-8 **1단계 머지됨**, 진행 중은 **2단계**

`develop` = `b2bce84` (2026-08-06 fast-forward 머지). `feature/table-selection-bar`는 삭제했다.
**미검증 커밋은 없다.** 현재 브랜치는 `feature/estimate-one-page-inline`.

## Step D7-8 2단계 — 「한 페이지 작성」 상시 안내

브랜치: `feature/estimate-one-page-inline`
규칙 출처: `sagetaechang-ui` > *MFC 공통 컨트롤 규격* · *크기와 좌표* · *진행바 / 상태 표시*
목업: 3-8의 선택 행

### 착수 전 목업·스킬 대조 (2026-08-06)

1단계에서 **세로 간격을 대조하지 않아** 표와 4px 겹친 일이 있었으므로, 이번에는 착수 전에 항목별로 훑었다.

| 항목 | 목업 | 판정 |
|---|---|---|
| 박스 | `border 1px #C9BFB1` · `radius 4` · `h32` · `bg #fff` | `BUTTON_BORDER` + `PANEL` 그대로 |
| 「한 페이지 작성」 / 「· 최대 6행」 | 13px / 12px `#7A7064` | `CONTENT`(105) / `CAPTION`(90) — 둘 다 있음 |
| 박스 좌우 여백 | **10** | **4의 배수 규칙 위반이라 8로 간다** |
| 세로 | 같은 행 h32 | 1단계에서 고친 리스트 시작점에 영향 없음 |
| 「선택 해제」 | **3-8에는 없다** (3-7에만) | 입력 표가 공용 인스턴스라 양쪽에 뜬다 — 의도적 이탈 |
| 인라인 경고 영역 | **없다.** 상시 힌트로 사전 차단만 한다 | 아래 결정 |

### 확정된 결정 (2026-08-06 사용자 결정)

**`AfxMessageBox` 3곳을 없애고 「· 최대 6행」 상시 표시로 대체한다.** 7번째 체크는 지금처럼 조용히
해제된다. `DESIGN_PLAN`은 「초과 안내를 인라인으로」라고 적었지만 **목업 3-8에 인라인 경고 영역이 없고**,
완료 기준도 「누르기 전에 안다 / 모달이 뜨지 않는다」이다. 인라인 줄을 신설하면 `CSageInlineError`의
패널 적용이 새 작업이 되고 자리를 상시 비워두느라 표가 내려간다. 조용한 해제가 불편하다고 판명되면
그때 피드백을 추가한다.

### 새로 만들 것

- `app/ui/drawing/SageOptionCheck.h/.cpp` — 테두리 박스 + 체크박스 + 라벨 + 힌트를 스스로 그리는
  오너드로우 체크박스. `vcxproj` · `filters` 3단계
- **`SageUiStyle::DrawCheckBox`** — 카멜색 체크박스 그리기를 `CSageListCtrl`에서 올린다.
  스킬이 *「두 컨트롤이 실제로 같은 코드를 가질 때만 모은다」*고 했고 이번이 그 조건이다
  (`SageUiStyle`에 조각이 둘이 되는 첫 사례)

### 작업 순서

- [ ] 1. `SageUiStyle::DrawCheckBox` 추출 + `CSageListCtrl`이 그것을 쓰게 한다 → 검증: 표 체크박스 무변화
- [ ] 2. `CSageOptionCheck` 신설 + 등록 3단계 → 검증: 빌드
- [ ] 3. 패널의 `m_wndOnePage`를 교체하고 힌트를 지정. `AfxMessageBox` 3곳 제거
      → 검증: 아래 *완료 기준*
- [ ] 4. `DESIGN_PLAN` D7-8 갱신 + `DEBT_LOG` → `docs:` 커밋

### 완료 기준

- 「최대 6행」을 **누르기 전에** 안다
- 6행 초과로 **모달 경고창이 뜨지 않는다** (`AfxMessageBox` 참조 0곳)
- 체크 상태 저장·복원이 그대로다 (워크플로 전환 왕복)
- **납품서에는 이 박스가 보이지 않는다** — 전환해서 확인한다 (1단계 교훈)
- 표 체크박스와 박스 안 체크박스가 **같은 모양**이다

### 범위 밖

선택 바의 「전체 선택」은 네이티브 체크박스로 남는다 — `DEBT_LOG`의 「체크박스가 한 화면에 두 종류」가
절반만 해소된다. 셋을 통일하려면 `CSageCheckBox`가 필요하고 그건 별도 작업이다.
