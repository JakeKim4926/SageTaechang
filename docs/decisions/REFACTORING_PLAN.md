...
디자인 결정은 `DESIGN_PLAN` D7-1에 있다.

**만든 것** — `app/ui/drawing/SageTableTotalBar.h/.cpp` (`CStatic` + `DrawItem`),
`ISageWorkflowHandler::BuildResultTotals` 확장점, 패널의 컬럼 인덱스 → 좌표 변환.

| 누가 | 무엇을 안다 | 무엇을 모른다 |
|---|---|---|
| `BuildResultTotals` | 어느 **컬럼 인덱스**에 어떤 값·역할 | 좌표 · 폭 |
| `SageResultTablePanel` | 인덱스 → **x좌표 · 폭 · 정렬** 변환 | 워크플로 |
| `CSageTableTotalBar` | 좌표 · 텍스트 · 역할대로 그리기 | 워크플로 · 컬럼 |

폭은 **`UpdateColumnWidths()` 안에서만** 다시 계산해 밀어넣는다. 밴드는 리스트를 참조하지 않고
컬럼 폭을 캐시하지도 않는다. 합산은 핸들러의 `SumVisibleRows` 한 곳에서 하고
요약 바와 밴드가 같은 결과를 쓴다.

**완료 기준 4항목 모두 달성** — 요약 바와 값 일치 · 창 크기 변경 시 컬럼 정렬 유지 ·
납품/견적에 밴드 없음 · 폭 결정 경로 1곳.

### 교훈

**리스트가 `WS_BORDER`라 컬럼 좌표가 창 rect보다 1px 안쪽이다.** 밴드를 `x=0`에 두면 셀이 1px
밀린다. `ClientToScreen` → `ScreenToClient`로 실제 오프셋을 구해 더했다 — **1을 상수로 박지 않았으므로**
테두리 스타일이 바뀌어도 따라간다. 표 위에 겹쳐 그리는 다음 요소(선택 바)도 같은 함정을 만난다.

**스킬을 고치면 계획만이 아니라 기존 구현과도 대조해야 한다.** 목업대로 muted 2색을 쓰기로 하고
`sagetaechang-ui`의 「섞어 쓰지 않는다」를 역할 표로 바꿨는데, 초안이 「라벨은 `#6E655B`」였다.
그대로 뒀으면 **1단계 요약 바(라벨 `#7A7064`)가 규칙 위반**이 됐다. 규칙을 고칠 때는 그 규칙이
설명해야 할 **기존 사례를 전부 대조**한다. `sagetaechang-plan`의 재점검은 계획 문서만으로 부족하다.

**목업 px를 그대로 옮기지 않는다는 규칙이 실제로 걸렸다.** 합계 행은 목업 12px지만 표의 일부이므로
표 셀 13px 계열(`SAGE_FONT_LIST_BOLD`/`SEMIBOLD`)을 썼다. 이 과정에서 스킬 *현재 구현된 설계 결정*에
남아 있던 낡은 폰트 값(표 셀 9pt · 캡션 8.3pt)이 타입 스케일 표(98 · 90)와 어긋난 것을 발견해
**참조 한 줄로 바꿨다** — 같은 수치를 두 곳에 적어서 갈라진 것이다.

남긴 표시 특성 4건(스크롤바 우측 여백 · 합계 0 표기 · 극단적 창 축소 · enum 2벌)은 `DEBT_LOG` 참조.
