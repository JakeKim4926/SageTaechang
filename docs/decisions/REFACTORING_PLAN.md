...
### 지금 어디인가 — **4d-2 완료. 남은 것은 4d-3(컨트롤러)뿐**

**미검증 커밋 1건** — `0f17bc4`. **빌드만 확인됐다.** 화면 확인 범위는 데이터 관리 탭 전체
(CRUD · 검색 · Enter · Tab · 카드 테두리 · 탭 재진입 시 목록 로드)다.

| 지표 | 4c 착수 전 | 4d-1 후 | **4d-2 후** | 목표 |
|---|---|---|---|---|
| 줄 수 | 1,873 | 1,492 | **1,025** | 600 내외 |
| 컨트롤 멤버 | 42 | 23 | **8** | 0 |
| 메시지맵 | 24 | 20 | **14** | 10 이하 |
| `app/infra` include | 6줄 | 6줄 | **5줄** | 0 |

**남은 컨트롤 멤버 8개는 전부 셸이다** — 사이드바 2 · 헤더 3 · 로고 1 · 인증 2.
**업무 컨트롤은 0개**가 됐다. 이후 줄이려면 3-B-5a(사이드바)·5b(헤더) 패널이 필요하다.

**`SageDBMgr.h` include가 View에서 사라졌다** — 기준 B(View는 SQL을 몰라야 한다)에서
View 자체는 DB 매니저를 모르게 됐다. 위반은 패널로 이동했고 `DEBT_LOG`에 올렸다.

**빌드 오류 1건을 냈다** — `OnWorkspaceTabChanged`에 `RefreshCompanyOrderList()` 호출이 남았다.
함수 정의만 지우고 호출부를 놓친 것이다. **옮긴 이름 30개로 전수 grep**해서 나머지가 없음을 확인했다.
**교훈: 정의를 지운 뒤에는 그 이름으로 전수 검색한다** — 컴파일러가 알려주기를 기다리면
한 번에 하나씩만 나온다.

**함께 고친 잠재 버그** — `OnWorkflowChanged`가 `RestoreWorkflowUiState`(→ `SelectTab`)를
`SetWorkflow`보다 먼저 불렀다. `SelectTab`은 `IsDataManageTab()`으로 목록 갱신을 판단하는데
그 시점 워크스페이스의 워크플로가 낡은 값이었다. **빌드로는 안 잡히고 「탭을 열었는데 목록이 비어 있다」로
나올 종류**였다. 순서를 뒤집었다.
...
| ~~**4d-2**~~ | ~~`SageCompanyOrderPanel` 신설 → 워크스페이스 하위 배치~~ — **완료** (`0f17bc4`, 화면 미검증) | 데이터 관리 탭 CRUD |
