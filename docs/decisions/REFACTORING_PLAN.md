...
**미검증 커밋 1건** — 4d-3. **빌드만 확인됐다.** 화면 확인 범위는 실행 경로 전체
(실행 · 완료 · 실패 · 초기화 · 드롭 · 워크플로 전환 시 상태 복원)다.
**특히 실행·초기화 버튼**은 링크 오류를 고치는 과정에서 「컴파일은 되지만 아무 일도 안 하는」
상태였던 것이 발견됐으므로 반드시 눌러 봐야 한다.
...
| ~~**4d-3**~~ | ~~`SageWorkflowController` 신설~~ — **완료** (화면 미검증) | 실행·완료·실패 경로 |
