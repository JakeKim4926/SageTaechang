...
**도달 기준** (`coding-design` > *ui 계층 구성*)

- [ ] `CSageTaechangView` 업무 컨트롤 멤버 **0개** (현재 104)
- [ ] `CSageTaechangView` 메시지맵 **10항목 이하** (현재 50)
- [ ] `CSageTaechangView` **600줄 이하** (현재 3,822)
- [ ] `View.cpp`에 워크플로 타입 분기 **0곳** (현재 36곳, 사이드바 등록 데이터 제외)
- [ ] `View.cpp`에 `#include "app/infra/..."` **0줄** (현재 6줄)
- [ ] 각 패널이 자기 컨트롤 · 메시지맵 · 레이아웃을 소유, View 위임 스텁 0개
...
3-B-4 ──→ 3-B-6  워커 결과를 받을 주체가 패널 HWND가 되므로
                 패널이 자리잡은 뒤에 실행 축을 뗀다

5   ←── 3-B · 4  새로 쓴 코드도 접두사 치환 대상이다
```

**순서 결정 근거**: Step 4는 View를 줄이는 작업이 아니다 (4-5는 View **+2줄**). 부피는
단가 패널 1,309줄 · 결과 표 393줄 · 레이아웃 233줄 · 법인 발주 198줄에 있고 전부 3-B 소관이다.
그래서 감량 효과가 가장 큰 단가 패널을 먼저 하고, 워크플로 축은 그 사이에 마무리한다.
...
| R7 | **워커 결과 수신처 변경** — `PostMessage(WM_TAECHANG_WORKFLOW_COMPLETE)` 대상이 View HWND다 | 전환 중 결과가 유실되면 진행바가 95%에서 멈춘 채 남는다 | 3-B-6에서 한 번에 전환. 그 전까지는 View가 받아 패널로 넘긴다 |
...
- [ ] 대상 집계 (컨트롤 · 메시지맵 · 함수 · 줄 수 · notification)
...
- [ ] `SageWorkflowPanel` 생성 — 입력 · 실행 · 결과 · 실행 기록. 문서 워크플로 3종 **공용**
...
### 3-B-5 — `SageSidebarPanel` · `SageHeaderPanel`

- [ ] 사이드바 트리 구성 · 선택 처리 이동 (워크플로 상수는 등록 데이터로 함께 이동)
- [ ] 헤더 제목 · 상태 · 인증 표시 이동
- [ ] 화면 확인: 업무 전환 · 로그인/로그아웃 · 비밀번호 변경 진입

### 3-B-6 — `SageWorkflowController` + Step 4-8

- [ ] 실행 상태 전이(대기 → 실행 → 완료/실패)와 워커 수명을 컨트롤러로
- [ ] 워커 결과 수신처를 패널 HWND로 전환 (R7)
- [ ] `ISageWorkflowRunner` 도입, infra 서비스 3개 구현, View의 infra include 6줄 제거
- [ ] 화면 확인: 워크플로 3종 실행 · 진행바 · 완료/실패 상태 · 연속 실행

**완료 기준**: 컨트롤러가 컨트롤 API를 호출하지 않는다. `View.cpp`에 `app/infra` include 0줄.
...
완료분은 *완료된 작업* 참조. 남은 넷은 3-B-3과 3-B-6에 삽입해 진행한다.
...
| 4-8 | 실행 축 + infra 역전 | 5곳 + include 6줄 | 3-B-6 |
...
3-B-6과 함께 진행한다. 위 3-B-6 체크리스트 참조.
...
- **사이드바 등록 데이터** — `BuildSidebarTree`가 워크플로 상수를 `SetItemData`에 싣는다.
  분기가 아니라 등록이므로 유지하고, 3-B-5에서 사이드바 패널로 함께 옮긴다
