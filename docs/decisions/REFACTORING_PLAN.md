...
> **먼저 확인을 받는다.** 브랜치 `feature/workflow-status-card`에 **빌드·화면 미검증 커밋**이 있다 —
> D7-4 2단계(상태 카드). D7-4 1단계는 **확인받았다.**
> 볼 곳: 미수금 입력 탭의 **대기 → 처리 중 → 완료** 3상태와 **실패** 1상태, 그리고
> 납품·견적 입력 탭(표가 38px 내려갔다). `DEBT_LOG` 참조.

| 순서 | 할 일 | 근거 |
|---|---|---|
| 1 | **D7-4 2단계 확인** — **앱을 새로 띄운 직후 화면부터** 보고, 4상태 + 납품·견적 입력 탭 | 미검증 커밋을 깔고 다음을 얹으면 실패 원인을 못 가른다. 첫 줄이 「시작 직후」인 이유는 `DEBT_LOG` 검증구멍 항목 |
| 2 | ~~D7-4 2단계~~ | **완료** 2026-08-07 · 미검증. `CSageStatusCard` 신설 |
| 3 | **D7-4 3단계** — 「폴더 열기」 · 「결과 보기」 | 위와 같은 절. **완료 카드 안 우측**에 30px Secondary 2개 |
| 4 | **D7-5**(실행 기록 5컬럼 표) → **D7-6**(데이터 관리 좌우 2단 + ↑↓) | 대상이 패널이 됐다. D7-6은 착수 전 결정 2건이 남아 있다(좌우 2단 구조 · 카드마다 Primary) |
...
**구조 리팩토링 쪽에는 미검증 커밋이 없다** (위의 미검증 1건은 디자인 Step D7-4 2단계다).
