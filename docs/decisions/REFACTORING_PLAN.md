...
### 부분검증 — 3-B-4a 재배선은 **빌드만 확인됐다. 화면 동작은 미확인이다**

**`develop`에 fast-forward 머지됨** (`9eeb313`·`62c8ea6`·`55c5ca4`, 2026-08-06 03:10~03:20).
작업 브랜치 `refactor/result-table-panel-wire2`는 삭제했다.

**빌드는 사용자가 확인했고 아래 9항목은 아직 아무도 실행해보지 않았다.**
이 변경은 **화면이 바뀌지 않아야 정상**이므로 눈으로 보는 것이 유일한 검증이다.
**다음 세션은 D5c로 넘어가기 전에 여기서부터 확인한다** — 위에 작업을 얹으면 실패 원인을 가릴 수 없다.
...
| 2 | **D7-1** 미수금 결과 — `CSageSummaryBar` 신설·배치를 **여기서 함께** | 아래 *D5c는 단독 Step이 아니다* |
| 3 | **D7-7 · D7-8** 납품·견적 — `CSageSelectionBar`를 **여기서 함께** | 같은 이유 |
| 4 | 나머지 D7 (7-2~7-6 · 7-9) | `DESIGN_PLAN` D7-2~D7-9 |
| 5 | 3-B-4b~4d (남은 패널 분리) | 아래 *4a가 남긴 것* |
...
따라서 **4a 확인 다음 실제 작업 단위는 D7-1**이고, 거기에 요약 바 · 합계 행 ·
