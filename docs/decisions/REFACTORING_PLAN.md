...
> **먼저 확인을 받는다.** `develop`에 **빌드·화면 미검증 커밋 2개**가 있다 —
> `35fb4e3`(D7-4 1단계) · `7e03122`(문서). **다음 작업을 얹기 전에** 빌드 + 화면을 확인받는다.
> `CSageSectionLabel`을 통째로 카드 헤더 규격으로 바꿔서 **사용처 6곳이 동시에 변했다** —
> 입력 카드 · 결과 표 제목 · 실행 기록 · 데이터 관리 2곳 · 단가 계산 기록. `DEBT_LOG` 참조.

| 순서 | 할 일 | 근거 |
|---|---|---|
| 1 | **D7-4 1단계 확인** (빌드 + 위 6곳) | 미검증 커밋을 깔고 다음을 얹으면 실패 원인을 못 가른다 |
| 2 | **D7-4 2단계** — 상태 카드 (대기/처리중/완료/실패 · 6px 진행바 · 「완료 · N건」 + 저장 경로) | `DESIGN_PLAN` D7-4. `app/ui/drawing/`에 새 컨트롤 |
| 3 | **D7-4 3단계** — 「폴더 열기」 · 「결과 보기」 | 위와 같은 절 |
...
**구조 리팩토링 쪽에는 미검증 커밋이 없다** (위의 미검증 2건은 디자인 Step D7-4다).
...
| 4 | **D7-4 · D7-5 · D7-6** | **진행 중** — D7-4 1단계 완료(**미검증**). 위 *다음 세션에 할 일* 참조 |
| 5 | **3-B-5a · 5b** 사이드바 · 헤더 패널 | View의 남은 워크플로 분기 9곳이 여기서 대부분 사라진다 |
