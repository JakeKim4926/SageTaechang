...
**4-5 이후 삽입** (`e8a0c56`) — View의 핸들러 조회를 `FindCurrentHandler()` 사설 헬퍼로 모았다.
4-4에서 4곳이던 것이 4-5에서 9곳이 되어 `DEBT_LOG`의 후속 조건("4-6 착수 전 결정")이 만기가 됐다.
NULL 검사는 반환값이 함수마다 달라 호출부에 남긴다. View.h는 전방 선언만 두고 핸들러 헤더를 물지 않는다.

> 일괄 치환이 헬퍼 자신의 본문까지 바꿔 **무한 재귀**가 됐던 것을 커밋 전에 잡았다.
> 문자열 치환 후에는 **정의부를 눈으로 확인한다.**
