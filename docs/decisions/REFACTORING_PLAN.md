...
### 지금 어디인가 — **4d-1 완료(화면 미검증). 다음은 4d-2**

**미검증 커밋 1건** — `4916e12`(브랜치 `refactor/workspace-panel`). **빌드만 확인됐고 화면은 아직이다.**
확인 범위는 워크플로 3종 × 탭 4개 = 12조합 + 워크플로 전환 시 탭 줄 흰 면이다.

| 지표 | 4c 착수 전 | 4c 완료 | **4d-1 후** | 목표 |
|---|---|---|---|---|
| 줄 수 | 1,873 | 1,629 | **1,492** | 600 내외 |
| 컨트롤 멤버 | 42 | 24 | **23** | 0 |
| 메시지맵 | 24 | 20 | **20** | 10 이하 |
| `UpdateTaskTabVisibility` | 47줄 | 17줄 | **11줄** | — |

**작업 중 낸 버그와 그 교훈** — 단가 화면 전환 시 탭 줄 흰 면의 좌우 24px이 남았다.
문서 워크플로에서 워크스페이스가 흰 면을 좌우 끝까지 칠하는데 **워크플로 전환 시 무효화를 하지 않았다.**
탭 컨트롤이 있던 가운데는 `ShowWindow(SW_HIDE)`가 부모 무효화를 일으켜 지워졌고 그 바깥만 남아
증상이 「좌우 흰 사각형」으로 보였다. `SetWorkflow`에 `Invalidate()`를 넣어 고쳤다.

**교훈 — 그리는 주체를 자식 윈도우로 옮기면 무효화 책임도 따라간다.**
D7-11에서 View가 그릴 때는 `DrawShellBands`가 `OnDraw`/`OnEraseBkgnd` 양쪽에서 매번 전체를
다시 칠했으므로 이 문제가 없었다. `WS_CLIPCHILDREN` 때문에 **부모의 `InvalidateRect`는 자식 영역을
건드리지 못한다** — 자식이 스스로 무효화해야 한다. 확인 목록에 「단가 화면」은 넣었지만
**「워크플로를 전환한 뒤」 보는 경로**가 빠져 있었던 것이 검증 설계의 구멍이었다.
