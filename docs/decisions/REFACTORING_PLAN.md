...
### 지금 어디인가 — D7-1 **1단계 머지됨**, 다음은 **2단계 합계 밴드**

`develop` = `7926a1c` (2026-08-06 fast-forward 머지). 작업 브랜치 `feature/receivables-summary-bar`는
삭제했다. 커밋 3개가 `feat`·`fix`·`docs` **세 목적**이라 squash하지 않고 보존했다
(`git-workflow` > *머지 전략 > 예외*).

**미검증 커밋은 없다.** 요약 바는 빌드+화면 확인, Excel 프로세스 종료는 생성 3회 연속 4초대 유지 확인,
3-B-4a는 화면 9항목 확인 완료.

현재 브랜치는 `feature/receivables-total-row` (PR_LOG 갱신 `docs:` 커밋 1개).

## Step D7-1 2단계 — 미수금 합계 밴드

브랜치: `feature/receivables-total-row`
규칙 출처: `sagetaechang-ui` > *표 렌더링 규격* · *색상 시스템* · *크기와 좌표*,
`coding-design` > *화면은 컨트롤을 그리는 방법을 몰라야 한다* · *신규 파일 추가 절차*
목업: `docs/design/sagetaechang-design-proposal.dc.html` 3-1 **하단 합계 행**

### 확정된 결정 (다시 묻지 않는다 — 2026-08-06 사용자 결정)

- **표 하단 고정 밴드로 그린다.** 리스트의 마지막 행(아이템)으로 넣지 않는다 — 필터·정렬·`〃` 그룹
  로직의 예외 케이스가 되고 스크롤하면 사라진다
- **보이는 행 기준으로 갱신한다.** 필터를 걸면 합계가 함께 줄어든다
- 폭은 **표 폭에서 파생**시킨다. 고정 폭 금지 (`sagetaechang-ui` > *크기와 좌표*)
- **색은 목업대로 muted 2종을 한 줄에 쓴다** — 「합계」 `TEXT_MUTED`(#6E655B) ·
  「N건」 `SECONDARY_TEXT`(#7A7064). 이 결정으로 `sagetaechang-ui`의
  「두 색을 섞어 쓰지 않는다」를 **역할별 열거로 교체했다**(스킬에 이력 기록).
  두 색의 채널 차이는 12뿐이라 실제 낙차는 굵기가 만든다
- **필터로 0건이 되면 밴드를 숨기지 않고 「합계 0건 · 0」을 표시한다** — 요약 바와 다르게 동작하면
  둘 중 어느 쪽이 맞는지 알 수 없다

### 목업 실측 (2026-08-06)

높이 40 · 배경 `LIST_HEADER`(#F2EEE7) · 상단 경계선 `BORDER` 1px · 셀 여백은 리스트와 동일
(`LIST_CELL_LEFT/RIGHT_PAD`). 목업은 9열이고 실제 표는 10열(「비고」)이지만 **앞 8열 순서가 같아
인덱스가 그대로 맞는다** (개선 전 캡처 `미수금_내역서_생성_결과.JPG`로 대조).

| 컬럼 | 내용 | 폰트 | 색 |
|---|---|---|---|
| 0 법인명 | 「합계」 | `LIST_BOLD` | `TEXT_MUTED` |
| 3 품목명 | 「N건」 | `LIST_SEMIBOLD` | `SECONDARY_TEXT` |
| 5 합계 금액 | 총액 합 (우측) | `LIST_BOLD` | `TEXT` |
| 6 입금 금액 | 입금 합 (우측) | `LIST_BOLD` | `TEXT` |
| 7 미수금 | 미수금 합 (우측) | `LIST_BOLD` | `PRIMARY` |

1·2·4·8·9는 빈칸. **폰트는 목업 12px가 아니라 표 셀 13px 계열을 쓴다** —
`sagetaechang-ui` > *타입 스케일*의 「목업 px를 그대로 상수로 옮기지 않는다」. 3종 모두 이미 있다.

### 1단계가 깔아둔 것 (그대로 쓴다)

- `TaechangResultRow`에 **금액 3종이 이미 `__int64`로 들어 있다** (`m_nTotalAmount` ·
  `m_nDepositAmount` · `m_nReceivableAmount`). 프리젠터가 파싱해 담는다. **새 파싱을 만들지 않는다**
- `SageWorkflowResultTable::FormatAmountNumber(__int64)`가 천단위 구분 문자열을 만든다
- `ISageWorkflowHandler::BuildResultSummary`가 **보이는 행을 받아 답하는 확장점**이다.
  합계 행도 같은 방식으로 핸들러에게 물어야 View에 워크플로 분기가 생기지 않는다
- 패널이 `GetVisibleRows()`로 필터 통과 행을 내주고, 필터 변경은
  `WM_TAECHANG_RESULT_TABLE_CHANGED` → `CSageTaechangView::UpdateResultSummary`로 이미 흐른다.
  **합계 밴드도 이 흐름에 얹는다**
...
| 누가 | 무엇을 안다 | 무엇을 모른다 |
|---|---|---|
| `ISageWorkflowHandler::BuildResultTotals` | 어느 **컬럼 인덱스**에 어떤 값·역할 | 좌표 · 폭 |
| `SageResultTablePanel` | 컬럼 인덱스 → **x좌표 · 폭 · 정렬** 변환 | 워크플로 |
| `CSageTableTotalBar` | 좌표 · 텍스트 · 역할대로 그리기 | 워크플로 · 컬럼 |

셀 역할은 `라벨 / 건수 / 금액 / 강조 금액` 4종 enum이다 — `CSageLabel`이 역할로 자기 색을 내는
기존 관용구를 따른다. 정렬은 새로 받지 않고 패널이 `m_arrColumns[i].nAlign`을 쓴다.

**요약 바와 합계 밴드는 같은 수치를 두 번 세지 않는다.** 핸들러 안에 합산을 1회 하는 private
헬퍼를 두고 `BuildResultSummary`·`BuildResultTotals`가 그 결과를 쓴다 (`debt-log-guard` *중복로직*).

### 작업 순서

- [ ] 1. `SageWorkflowResultTable.h`에 `SageResultTotalCell` + 역할 enum (`SAGE_TOTAL_CELL_*`)
      → 검증: 컴파일
- [ ] 2. `ISageWorkflowHandler::BuildResultTotals` + 핸들러 3개 (미수금 실구현, 납품·견적 `FALSE`).
      구분 표기(`-`) 행은 건수에서 제외 — 요약 바와 같은 규칙
      → 검증: 요약 바의 총 건수·미수금 합계와 밴드 값이 **항상 일치**
- [ ] 3. `app/ui/drawing/SageTableTotalBar.h/.cpp` 신설 + `vcxproj`·`filters` 등록 3단계
      → 검증: 빌드
- [ ] 4. 패널 배선 — 하단 밴드 배치, 리스트 높이에서 밴드 높이 차감, 인덱스→좌표 변환.
      View의 `UpdateResultSummary`에 합계 갱신 추가
      → 검증: 아래 *완료 기준* 4항목
- [ ] 5. `DESIGN_PLAN` D7-1 갱신 + `DEBT_LOG` 정리 → 별도 `docs:` 커밋

코드는 **`feat:` 커밋 1개**로 묶는다. 단계별로 쪼개면 컴파일되지 않는 중간 커밋이 생긴다.

### 완료 기준

- 요약 바의 총 건수·미수금 합계와 밴드의 값이 **항상 같다** (필터·창 크기와 무관)
- 창을 최소~최대로 끌어도 밴드 금액이 **컬럼과 어긋나지 않는다**
- 납품·견적 결과에는 밴드가 **없다** (핸들러가 `FALSE`)
- 밴드가 컬럼 폭을 **자체 판단하지 않는다** — 폭이 정해지는 경로가 `UpdateColumnWidths()` 하나뿐이다

### 주의 — 컬럼 폭은 런타임에 바뀐다

`UpdateColumnWidths()`가 `MulDiv`로 재배분하므로 밴드가 컬럼 폭을 캐시하면 창 크기 변경 시 어긋난다.
**밴드가 리스트를 직접 참조하지는 않는다** — 그리기 위젯이 리스트를 알게 된다.
대신 **폭이 바뀌는 유일한 지점인 `UpdateColumnWidths()` 안에서** 패널이 실측 폭으로 셀 좌표를 다시
계산해 밀어넣는다. `Layout` · `SetColumns` · 창 크기 변경이 모두 이 함수를 지나므로 어긋날 경로가 없다.

### 알려진 표시 특성

리스트 세로 스크롤바가 뜨면 리스트 클라이언트 폭이 패널 폭보다 좁아진다. 밴드는 리스트와 같은 원점·
같은 실측 폭을 쓰므로 셀은 맞지만 **우측 끝 몇 px이 밴드 배경으로 남는다.** 맨 끝 「비고」 열은
합계 행에서 빈칸이라 값이 어긋나 보이지 않는다.

행이 적으면 **마지막 행과 밴드 사이에 빈 공간이 넓게 남는다** (개선 전 캡처에서 확인). 목업은 행이
표를 꽉 채운 상태라 이 모습이 보이지 않는다. 하단 고정이 확정된 결정이므로 그대로 간다.

### 남은 D7-1 범위 밖 항목

「내보내기」 버튼과 D6이 넘긴 아이콘 2종(내보내기·성공)은 **사용자 결정으로 만들지 않았다.**
`DEBT_LOG` 2026-08-06 항목 참조 — 내보내기의 동작을 먼저 정해야 한다.
...
| 순서 | 할 일 | 근거 |
|---|---|---|
| 1 | **D7-1 2단계** 합계 밴드 | 위 *Step D7-1 2단계* |
| 2 | **D7-7 · D7-8** 납품·견적 — `CSageSelectionBar`를 **여기서 함께** | 아래 *D5c는 단독 Step이 아니다* |
