...
### 지금 어디인가 — D7-7 · D7-8 **1단계 완료·검증됨**, 다음은 **2단계**

`develop` = `de346ed`. 현재 브랜치 `feature/table-selection-bar`에 커밋 6개가 있고
**아직 `develop`에 머지하지 않았다.** 빌드 + 화면 확인 완료(납품·견적 전환 포함).
**미검증 커밋은 없다.**

## D7-7 · D7-8 1단계 — 표 선택 바 **완료** (2026-08-06)

입력 표 상단에 「전체 선택 · N건 중 M건 선택됨 · 선택 해제」를 두고, 생성 버튼 문구에 선택 건수를
반영하며 0건이면 비활성으로 만든다. **입력 표는 납품·견적 공용 인스턴스라 두 화면에 함께 붙었다.**

**만든 것** — `CSageSelectionBar`(체크박스·「선택 해제」를 소유하고 클릭은 커맨드 ID로 부모에 올린다),
`ISageWorkflowHandler::BuildActionButtonLabel`, `WM_TAECHANG_RESULT_SELECTION_CHANGED`,
`SAGE_FONT_CONTENT_SEMIBOLD`(105 크기 SemiBold가 없었다).

체크 변경은 행 집합 변경과 **분리된 메시지**로 흐른다 — 체크 토글마다 요약 바·합계 밴드를 다시
계산할 이유가 없다. 대량 `SetCheck` 구간은 알림을 억제하고 끝난 뒤 한 번만 보낸다.

### 함께 고친 것 — 표 체크박스가 보이지 않던 D4 회귀

`f4690b8`(행 높이 34) 이후 납품·견적 입력 표의 체크박스가 보이지 않았다. 그 화면을 그 뒤로
눈으로 확인한 적이 없어 드러나지 않았다. `CSageListCtrl`이 상태 이미지 2장을 직접 그려
`LVSIL_STATE`에 지정하도록 바꿨다(목업대로 카멜색). **확인 빌드를 4회 쓴 작업이고 교훈이 셋이다.**

### 교훈

**상태 이미지리스트는 컨트롤이 소유한다.** `LVS_EX_CHECKBOXES`가 꺼지는 순간 comctl32가 붙어 있던
상태 이미지리스트를 파괴한다. `SetColumns`가 매번 확장 스타일을 통째로 덮어써서 이 일이 반복되는데,
멤버로 들고 재사용하면 **두 번째 워크플로부터 죽은 핸들을 물게 된다.** 매번 새로 만들어 지정하고
`Detach`로 소유권을 넘긴다. 「첫 화면만 되고 전환하면 안 된다」는 증상이 이 종류의 신호다.

**상태 이미지 인덱스는 1-based다.** 해제 = 상태 1 = 이미지리스트 **0번**, 선택 = 상태 2 = **1번**.
0번을 빈 이미지로 두면 「해제는 안 보이고 선택하면 빈 박스가 뜨는」 증상이 나온다.

**원인을 확정하기 전에 고치지 않는다.** 같은 증상에 세 번 손댔고 앞의 둘은 추정이었다.
증상에서 역산할 수 있는 조합(위 인덱스 건)이나 소유권처럼 **코드로 확인 가능한 사실**을 먼저 잡는다.

**완료 기준을 수정에도 적용한다.** 계획서에 「납품·견적 두 화면에서 같게 동작한다」를 적어놓고
체크박스 수정에는 그 기준을 쓰지 않아, 한 화면만 보고 완료로 넘겼다가 되돌아왔다.

**목업 대조는 가로만이 아니다.** 색·폰트·컬럼은 실측해 표로 옮겼으나 **세로 간격을 대조하지 않아**
밴드 행(32px)이 표(할당 26px)와 4px 겹친 것을 놓쳤다. 미수금은 요약 바가 밀어내서, 납품·견적은
겹치는 폭이 버튼 하나뿐이라 지금까지 가려져 있었다.

### 범위 밖 (2단계 `feature/estimate-one-page-inline`)

- 「· 최대 6행」 상시 표시와 `AfxMessageBox` 3곳 → 인라인 안내
- **목업 3-8의 「한 페이지 작성」은 맨 체크박스가 아니라 테두리 박스**(1px `#C9BFB1`, radius 4, h32)
  안에 들어 있다. 1단계 착수 전 재대조에서 찾았다
- 생성 버튼을 선택 바로 옮기는 것은 3-B-6a 이후 재검토한다
