...
아래 *앞으로의 순서* 표에서 **1번 또는 2번**을 집으면 된다. 3번(패널 분리)을 먼저 하면
2번의 일부(D7-4~D7-6)가 함께 풀린다. 착수 전에 `sagetaechang-plan`대로 Step을 이 문서에 적고,
...
| 순서 | 할 일 | 준비 상태 · 근거 |
|---|---|---|
| 1 | **`fix/large-table-render`** — 690행 표 렌더 580ms | **측정까지 끝났다.** 구간별 수치와 후속 2단계가 `DEBT_LOG`에 있다. 범위는 `RefreshRows` 한 곳 |
