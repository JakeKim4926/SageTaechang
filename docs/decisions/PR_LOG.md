...
- **PR 링크**: 없음
- **결과**: merged — `develop`에 fast-forward(`63da396`). 커밋 5개 보존, 작업 브랜치 삭제. 빌드·화면 확인 완료
- **남긴 것**: 목업 실측 5건(입력 테두리 `#C9BFB1` · 결과 카드 top 패딩 8 · 표 셀 「원」 제거 · 합계 열 Bold · 초기화 버튼 테두리)은 **스킬 개정이나 다른 화면의 표에 영향이 있어** 계획서 D7-12 *남은 목업 차이*에 등재했다. 부채 4건은 `DEBT_LOG`
- **되돌아보면**: 착수 조사에서 헤더 밴드가 테두리를 덮는 것을 **보고도 넘겼다** — *다른 패널도 같으니 일관적*이라고 판단했다. 실제로는 세 화면이 똑같이 목업과 달랐던 것이고, **코드끼리 대조하고 목업을 열지 않은** 결과다. D7-11의 「스킬을 목업의 대체물로 썼다」와 같은 종류다. 「기존 화면과 같다」는 목업과 맞다는 근거가 아니다. 또 하나 — D7-2가 배치를 미룰 때 **번호를 붙이지 않아** 진행 상황 표에서 보이지 않았고 D7이 닫히며 함께 묻혔다. **미루는 순간 Step 번호를 붙인다**
...
- **PR 링크**: 없음
- **결과**: merged — `develop`에 fast-forward(`9497618`). 커밋 4개 보존, 작업 브랜치 삭제. 빌드·화면 확인 완료
- **되돌아보면**: **한 카드에서 다섯 번 틀렸다** — 변수 중복 선언(빌드 오류) · 입력 정렬 · 저장 버튼 변형 누락 · 체크박스 들여쓰기 · 버튼 줄바꿈. 원인이 하나다: **목업 원문을 끝까지 읽지 않고 `grep` 요약만 봤다.** 잘린 속성(`text-a…`), `padding-left:76px`, `<div>` 닫힘 위치가 전부 요약에서 빠졌다. 구조가 있는 블록은 요약이 아니라 그 구간 원문을 그대로 읽고 손댄다

## [2026-08-19] feature/msgbox-focus-ring
- **목적**: 목업 3-10의 「기본 포커스(연한 링)는 안전한 쪽에 둔다」를 화면에 드러낸다
- **변경 내용**: `CSageButton`이 `ODS_FOCUS`를 읽어 링을 그리도록 하고(기본값 꺼짐), 메시지 박스의 확인 창에서 기본 포커스 버튼에만 켠다. 링이 테두리 **바깥** 2px이라 오너드로우로는 그릴 수 없어, 그 버튼만 창을 상하좌우 2px 키우고 안쪽에 그린다 — 보이는 크기는 그대로다. 링 색은 변형이 정한다(Primary `#F0E4D5` · 그 외 `#EFEBE3`)
- **PR 링크**: 없음
- **결과**: merged — `develop`에 fast-forward(`0951c8a`). 커밋 1개, 작업 브랜치 삭제. **빌드·화면 확인 전**
- **되돌아보면**: 착수 전 이 항목을 「`CSageButton` 전역 기능」으로 제시했는데, 목업을 열어보니 **3-10 두 곳뿐**이었다. 범위를 말하기 전에 목업에서 해당 표현이 몇 곳에 쓰였는지 먼저 센다

## [2026-08-19] feature/company-picker-list
- **목적**: 법인 선택 목록을 목업 렌더링으로 바꾸는 것으로 시작했으나, 사용자 확인 과정에서 드러난 정렬·색·기록·검색 문제를 이어서 처리했다
- **변경 내용**: (1) `CSageListBox` 신설 — 법인 선택 목록이 Windows 기본 파란 선택색을 쓰고 있었다. (2) 폼 정렬 3종 — 라벨 열 보정값 제거 · 라벨/콤보/버튼 세로 중심 통일 · 콤보 닫힌 높이를 `COMBOBOXINFO`로 재서 32에 맞춤. (3) 실행 기록을 산출물 단위로 — `files` 배열을 순회해 파일당 1행. (4) 표 선택 행을 목업 신판 색 `#F1E3CD`로, 액센트 바를 표 4px/사이드바 3px로 분리. (5) `CSageSearchBox`에 기준 칸 변형을 추가해 표 필터와 법인 선택 검색을 통합 박스로 교체, 필터 뒤 흰 카드 제거. 목업 사본을 원본 최신판으로 두 번 갱신했다(3-10 신설 · 초기화 버튼 추가분 반영)
- **PR 링크**: 없음
- **결과**: merged — `develop`에 fast-forward(`dd2c56c`). 커밋 23개 보존, 작업 브랜치 삭제
- **되돌아보면**: **브랜치 하나에 작업 단위가 여러 개 섞였다.** 화면을 확인받는 과정에서 인접한 결함이 계속 드러났고 같은 브랜치에서 이어 고쳤다. 결과적으로 `git-workflow`의 「브랜치 하나 = 작업 단위 하나」를 지키지 못했다. 확인 중 새 결함이 나오면 **그 자리에서 브랜치를 나눌지 먼저 정한다**

## [2026-08-19] feature/sage-message-box
- **목적**: `AfxMessageBox`를 앱 스타일 모달로 교체한다 (D9 1단계)
- **변경 내용**: `SageMessageBoxDlg`(`SageFramelessDialog` 파생)와 `CSageMessageBody`(아이콘 3종 + 워드랩 본문을 자기가 그리고 높이를 계산)를 신설하고 진입 함수 `ShowSageMessageBox`를 뒀다. 호출부는 창 크기 계산을 먼저 확인하려고 `SageSidebarPanel` 1곳만 바꿨다(잔여 55곳). 예/아니오 창에 `IDOK` 버튼이 없는데 Enter가 `IDOK`을 보내는 구멍을 `OnOK` 오버라이드로 막았다 — 계획서에는 Esc·X만 적혀 있었다. 본문은 목업 대조에서 흰 면으로 정정
- **PR 링크**: 없음
- **결과**: merged — `develop`에 fast-forward(`49fcedb`). 커밋 2개 보존, 작업 브랜치 삭제. **빌드·화면 확인 전**

## [2026-08-19] feature/dialog-white-body
- **목적**: 다이얼로그를 목업 3-9와 맞춘다 (D7-9 후속)
- **변경 내용**: 다이얼로그 8종의 본문을 `PANEL`(흰 면)로 바꾸고 라벨 20개·인라인 에러 8개에 배경 역할을 지정했다(`CSageInlineError`에 `SetBackgroundRole` 추가, 기본값은 기존대로 `APP`). 이어서 단가 추가를 목업 배치로 다시 썼다 — 라벨-입력 가로 한 줄, 입력 폭 110/130, 「부」·「원」 단위 라벨, 숫자 우측 정렬, 체크박스 2개를 한 행으로 모아 `CSageOptionCheck`로 교체(테두리 표시 여부 속성 추가, 기본값 TRUE라 견적서 「한 페이지 작성」은 그대로). `int nCheckW = 110` 하드코딩이 함께 사라졌다
- **PR 링크**: 없음
- **결과**: merged — `develop`에 fast-forward(`f172354`). 커밋 3개 보존, 작업 브랜치 삭제. **빌드·화면 확인 전**

## [2026-08-19] docs/design-messagebox-plan
- **목적**: 모달 메시지 박스 교체를 `DESIGN_PLAN`에 Step D9로 등재한다
- **변경 내용**: 조사로 `AfxMessageBox` 실사용이 9파일 56곳임을 확정하고(R7의 「11파일 90곳」과 `DEBT_LOG`의 「View 18곳」이 낡았다) 설계·4단계·완료 기준·리스크를 적었다. 2차 점검에서 스킬 위반 2건(파일명이 클래스명과 다름, 스킬 갱신 대상이 컨트롤 목록 표)과 설계 구멍 1건(`nType`만으로는 삭제 확인을 판정할 수 없다)을 찾아 정정했다
- **PR 링크**: 없음
- **결과**: merged — `develop`에 fast-forward(`a81c6a2`). 커밋 2개 보존, 작업 브랜치 삭제

## [2026-08-13] fix/input-file-workflow-mismatch (폐기)
- **목적**: `DEBT_LOG` 2026-08-06 「입력 파일이 워크플로와 맞는지 검사하지 않는다」를 닫는다. 열린 부채 중 유일하게 실사용 피해가 확인된 항목이었다
- **변경 내용**: 핸들러가 기대 숫자 열(JSON 키)을 답하고 core의 `SageWorkflowInputCheck`가 로드 응답의 rows를 대조하는 구조를 만들었다(`18a48c0`, 17파일). **사용자 확인에서 정상 납품서 파일이 막히는 것이 드러나 전부 철회했다.** 원인은 납품서 부수 3열(`회사부수`·`법인부수`·`총부수`)이 실제로는 자유 텍스트라는 것이다 — 백업 파일 실측값이 `10부` · `각7부` · `X` · `여분(2부)` · 빈칸이고, 열 전체가 비면 실패로 판정하는 코드까지 겹쳐 **정상 파일이 100% 차단**됐다. 견적서·미수금 샘플은 통과했는데 그건 그 두 워크플로의 값이 우연히 진짜 숫자였기 때문이고, **「워크플로마다 숫자 열이 있다」는 전제 자체가 성립하지 않았다.** 대안으로 ① 헤더 문구 대조(부채의 원래 후속안) → **헤더는 파일마다 달라질 수 있다**는 사용자 지적으로 폐기, ② 세 서식 중 최근접을 고르는 상대 판정(셀 서식으로 날짜 열 판별 + 데이터 시작 행 · 마지막 사용 열 · 개행 포함 텍스트 열 지문, 임계값 없음)을 설계했으나 **검사 자체를 빼는 것으로 결정**됐다. **실수는 검증 순서였다** — 불일치 케이스(미수금→견적서)만 검증하고 정상 케이스를 실제 파일로 확인하지 않았다. 3파일 중 1개만 열어봤어도 착수 전에 드러났다. 커밋 전수를 목적별로 분류해 **검사와 무관한 것은 `SplitJsonObjectArray` 승격 1건뿐**임을 확인하고 그것만 따로 살렸다(아래 항목)
- **PR 링크**: 없음
- **결과**: **폐기** — 브랜치 삭제, `develop` 미반영. 커밋 `18a48c0`은 reflog에만 남는다. `DEBT_LOG` 항목은 **닫지 않고** *시도와 철회*를 붙여 열어 두었다
...
- **변경 내용**: 둘 다 단일 커밋이라 fast-forward로 넣고 브랜치를 삭제했다(`775f5a8` → `48e5aa1`). `docs/debt-log-count`는 `develop`보다 3커밋 뒤처져 있어 리베이스 후 머지했다. **문서 전용 브랜치의 머지 결과를 기록하려고 또 문서 브랜치를 만들지 않는다** — `docs/pr-log-d8-merge`가 그 한 바퀴였고 그대로 두면 기록이 무한히 뒤따라온다. **앞으로 문서 브랜치의 머지는 다음 작업 브랜치의 `PR_LOG` 커밋에 함께 적는다**
- **PR 링크**: 없음
- **결과**: merged (`develop` = `48e5aa1`)

## [2026-08-10] docs/d8-hold
- **목적**: 「다음은 D8(DPI 배율)」로 남아 있던 계획서 1순위를 정리한다. **코드 변경 없음**
- **변경 내용**: 착수 여부를 판단하려고 **실제 배율을 먼저 물었고 작업 PC가 100%로 확인**됐다. 배율 1.0에서는 Windows가 DPI 가상화를 하지 않으므로 **D8이 고칠 증상(흐릿함) 자체가 존재하지 않는다** — 얻는 것이 0이고 R11(폰트만 커지고 좌표는 그대로)만 떠안는 상태라 보류로 옮겼다. 재개 조건을 **「배포 대상 PC에 125% 이상이 등장하면」**으로 명시했고, 그 PC에서는 지금도 흐릿하게 보일 것이므로 **증상으로 판별할 수 있다**고 적었다. 다른 설치 PC의 배율은 **미확인**이라는 것도 전제로 남겼다. **「manifest만 켜는 부분 착수 금지」를 문서에 못박았다** — 반쪽 적용이 지금보다 나쁜 화면을 만든다. **제안받은 「D8-0: DPI 준비 감사/정리」 Step은 만들지 않았다** — 코드를 전수로 확인하니 C9-2가 D1 착수 시점에 건 규칙(「좌표에 하드코딩 숫자를 만들지 않는다」)이 **D1~D7 내내 실제로 지켜져 있었다**: `MoveWindow`/`SetWindowPos` 185곳의 픽셀 리터럴 **0건**(걸린 8건은 `* 2` 배수 연산), 레이아웃 지역변수의 숫자 33건 전부 `* 2`·`/ 2` 비율 연산, `app/ui/drawing/`의 그리기 좌표 하드코딩 **0건**. 즉 「나중에 스케일을 끼울 수 있는 상태」는 이미 만들어져 있어 감사 Step은 끝난 일을 다시 확인하는 작업이 된다. 미확인으로 남은 것은 *치수 상수 228개의 스케일 대상 분류* 하나뿐이고 **지금 만들면 낡으므로** 착수 시점으로 미뤘다. 비운 1순위 자리는 **입력 파일 워크플로 검증**(열린 부채 중 유일하게 실사용 피해가 확인된 항목)으로 채웠고, **등재 13번(교대 행 그룹 단위)은 D4c 철회로 무효가 됐을 수 있어 「생존 확인」을 별 항목으로** 세웠다
- **PR 링크**: 없음
- **결과**: merged — 문서 전용(빌드 대상 아님). **로컬 merge commit `fd0b262`이고 시각을 2026-08-11 23:20:37로 지정했다**(사용자 결정). `git-workflow`의 squash 기본·develop PR 반영 두 규칙의 예외이며, 히스토리의 머지 시각은 실제 머지 시점과 다르다

## [2026-08-09] fix/list-imagelist-ownership
- **목적**: `SetColumns`가 확장 스타일을 통째로 덮어쓰던 것을 고치고, 그 전제 위에 세워져 있던 상태 이미지리스트 우회책을 정리한다. `DEBT_LOG` 2026-08-06 항목을 닫는 작업
- **변경 내용**: 착수 전 **범위 지시안을 두 번 검토했다.** 첫 지시안은 「1px 스페이서까지 제거」였으나 과했고, 두 번째는 범위가 맞았지만 **`SetCheckboxes`의 이미지리스트 수명이 빠져 있었다** — `SetColumns`가 더 이상 `LVS_EX_CHECKBOXES`를 끄지 않으면 파괴 경로가 사라지므로, **매번 새로 만들어 교체하는 구조가 그 순간부터 누수가 된다**(`LVM_SETIMAGELIST`는 이전 것을 반환하고 정리는 애플리케이션 몫). 세 번째 지시안이 `OnDestroy` 분리까지 더해 확정됐고, 그것도 정확했다 — **`m_imgRowSpacer`는 이미 이중 해제 위험이 있었다.** 멤버로 소유하면서 리스트뷰에 붙인 채 둬서 comctl32와 `CImageList` 소멸자가 같은 핸들을 두 번 파괴할 수 있었고, 앱 종료 시점이라 증상이 안 보였을 뿐이다. **작업 중 회귀 1건을 냈다**: 「스타일을 먼저 켜면 comctl32가 기본 이미지리스트를 만들어 누수된다」를 **순서를 바꿔서** 피하려 했는데(붙이고 나서 켜기), `LVS_EX_CHECKBOXES`가 꺼진 상태에서 붙은 `LVSIL_STATE`는 **리스트뷰가 영역 폭을 확보하지 않아 클릭 판정이 0**이 됐다. 그림은 `ImageList_GetIconSize`로 폭을 읽어 그리므로 **보이는데 눌리지 않는** 상태가 됐고, **이 표가 오늘 아침 겪은 버그와 증상이 같아** 사용자가 캡처로 잡아줬다. 순서를 되돌리고 **반환 핸들을 파괴하는 방식**으로 바꿨다 — 우리 멤버와 핸들이 같으면 남기고 다르면 파괴하는 비교가 「내 것인지 남의 것인지」의 기준이다. 떼는 판단이 `SetCheckboxes(FALSE)`와 `OnDestroy` 두 곳에 필요해 `DetachStateImages()`로 묶었고, 이전 핸들을 그대로 받으려고 MFC 래퍼 대신 `ListView_SetImageList`를 썼다
- **PR 링크**: 없음
- **결과**: 빌드·화면 확인 완료(체크박스 표시 · 클릭 건수 증가). 머지 대기
