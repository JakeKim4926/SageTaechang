## [2026-08-08] style/table-center-align
- **목적**: 표가 넓어지면서 좌우 정렬이 한쪽 끝에 붙어 보인다는 지적에 따라 셀을 가운데로 통일한다
- **변경 내용**: 사용자가 미수금·납품서·견적서·법인 데이터 넷을 지목했으나, 스킬이 「**모든 표가 규격을 공유한다. 화면마다 다르게 그리지 않는다**」를 CRITICAL로 두고 있어 **단가 표 2종과 실행 기록까지 7개 전부** 맞췄다. **금액 열은 우측 유지를 제안했다** — 자릿수가 줄을 맞춰야 비교되고 Pretendard를 쓰는 이유가 그것인데, **전부 가운데로 결정**받아 부채로 등록했다. 걸림돌은 **Win32가 0번 열을 항상 좌측으로 고정**하는 것이라 표 6곳에 `SetFirstColumnAlign`으로 커스텀드로우를 켰다. 미수금 결과 표의 0번 열은 **그룹 열**이기도 해서 `DrawGroupColumn`의 하드코딩된 `DT_LEFT`도 고쳤고, **합계 바**가 `bRightAlign` BOOL 대신 `SageColumnAlign`을 받도록 바꿔 금액 열 아래 숫자가 셀과 어긋나지 않게 했다. 스킬의 *헤더 정렬* 절은 「셀 정렬은 목업대로」라고 적혀 있어 **거짓이 됐으므로** *정렬* 절로 다시 쓰고 이력을 남겼다. **작업 중 실수 1건**: `sed -i`로 정렬 상수를 일괄 치환해 **CRLF 646줄이 또 날아갔다** — **네 번째**이고, 「한글 든 파일은 Edit 도구로」를 전날 부채에 써두고 어겼다
- **PR 링크**: 없음
- **결과**: 빌드·화면 확인 완료(표 7종 · 합계 바 · 체크박스 열 · 그룹 열 `〃`). 머지 대기

## [2026-08-08] refactor/header-panel
- **목적**: 3-B-5b — 헤더를 `SageHeaderPanel`로 분리하고 목업 3-1 헤더를 함께 맞춘다. **3-B의 마지막 Step**
- **변경 내용**: 착수 전 결정 **R9**가 「제거」로 정해졌다 — `m_wndHeaderStatus`는 `WS_VISIBLE`도 없고 `MoveWindow(0,0,0,0)`인데 멤버 3 · 함수 2 · `OnCtlColor` 분기 1이 딸려 있었고, 상태 표시가 **프레임 상태바 · D7-4 상태 카드 · 헤더 상태**로 세 겹인데 셋째만 안 보였다. 함께 `m_nAuthDividerX`와 **인증 구분선 그리기**도 걷어냈다(목업 헤더에 구분선이 없다). **5a와 달리 커밋을 나누지 않았다** — 「관리자」 pill과 「문서 생성」 보조 라벨은 옮기는 컨트롤 자체가 바뀌는 것이라 옮긴 뒤 고치면 두 번 쓴다. **`CSageBadge` 신설**: 목업 pill 색이 전부 기존 상수여서 색을 인자로 받게 두었고 등재 8번(「기타 처리」 배지)에 재사용된다. **보조 라벨은 핸들러가 아니라 사이드바에서 받는다** — `GetSelectedCategory()`. 핸들러에 메서드를 더하지 않으려는 선택이고, 핸들러가 없는 가격 워크플로도 그룹은 나온다. **View 411 → 299줄, 업무 컨트롤 멤버 0개.** 통지 소비자 대조 재실행, 끊김 0
- **PR 링크**: 없음
- **결과**: 빌드·화면 확인 완료(흰 면·구분선 · 로그인/로그아웃 · 관리자 pill · 보조 라벨 · 창 크기 변경). 머지 대기

## [2026-08-08] refactor/sidebar-panel
- **목적**: 3-B-5a — 사이드바를 `SageSidebarPanel`로 분리하고 목업 3-1 상단을 함께 맞춘다
- **변경 내용**: 착수 전 **4d-3이 회귀 2건을 낸 것과 같은 종류의 이동**이라 `DEBT_LOG` 검증구멍 항목을 먼저 읽고, **이동 직후 통지 소비자를 전수 대조**했다(수신처 없는 통지 0건). 트리 구성과 **워크플로 등록 데이터 9곳**, 로그인 필요 판정, 선택 복원이 패널로 갔고 View는 통지 둘만 받는다 — **View 493 → 411줄, 워크플로 상수 참조 9 → 3곳.** 사용자 결정으로 목업 반영을 같은 브랜치에서 했지만 **커밋을 「이동」과 「목업 반영」으로 나눠** 원인을 가를 수 있게 했다. 착수 전 결정 2건 중 **R9(헤더 상태 컨트롤)는 「제거」로 정해졌고 5b에서 처리**한다. **화면 확인에서 셋이 나왔다**: ① 트리를 폭 전체로 넓히며 `WS_BORDER`가 드러나 구분선이 두 줄로 보였다(내 회귀) ② 그룹 라벨이 항목과 같은 14px(목업 11px → 스킬 캡션 12px) ③ 선택 항목이 흰색·SemiBold가 아니었다. **②③은 등재 2번이 「letter-spacing」만 적어둔 탓에 안 보던 차이**이고, 내가 계획서만 보고 「거기 적힌 것만」 확인한 결과다 — 사용자가 **「목업과 스킬을 더블체크해서 작업한 게 맞느냐」**고 물어 드러났다. 마지막으로 **로고·그룹 라벨·항목의 좌측이 x=20으로 같아야 하는데** 트리가 자식을 들여쓰기해 맞출 수 없었고, 트리를 밀면 **선택 배경과 카멜 바까지 밀려서**(목업은 폭 전체) 결국 커스텀드로우에서 배경·바·텍스트를 직접 그리고 `CDRF_SKIPDEFAULT`로 막았다. 그 과정에서 letter-spacing도 해결되어 **등재 2번이 완전히 닫혔다**
- **PR 링크**: 없음
- **결과**: 빌드·화면 확인 완료(업무 전환 · 선택 표시 · 로그인 필요 안내 · 비밀번호 변경 복귀 · 좌측 정렬). 머지 대기

## [2026-08-08] style/company-order-two-column
- **목적**: D7-6 — 데이터 관리를 목업 3-6의 좌우 2단으로 바꾸고 순서 이동을 넣는다. **디자인의 마지막 Step**
- **변경 내용**: 착수 전 목업을 재면서 **계획서의 착수 전 결정 2건 중 하나가 이미 끝나 있는 것**을 찾았다 — 「카드마다 Primary」는 D7-4 때 스킬을 카드 단위로 완화했는데 계획서만 낡아 있었다. 남은 것은 좌우 2단이었고, 재면서 하나가 더 나왔다: **「이동」 열의 ↑↓를 행 안에 못 넣는다**(`CListCtrl` 행에 버튼 불가 + 텍스트로 그려 클릭받으면 스킬의 Text/Link 금지) — D7-5에서 동작 열을 뺀 것과 같은 이유라 **편집 카드의 순서 칸 옆**으로 옮겼다(사용자 결정). **「수정」 버튼이 없어졌다** — 목업은 행 선택이 곧 편집이고 그 동작은 `OnListSelChanged`에 이미 있었다. 상태가 `IDLE`/`MODIFY` → 「선택 기반」/「새 항목」으로 정리됐다. **순서 교환이 그냥 되지 않았다**: `ChangeCompanyOrder`가 중복을 막아 두 번 호출하면 첫 호출에서 충돌한다. 스키마에 `sort_order` UNIQUE가 없어(인덱스만) **리포지토리에서 트랜잭션으로 두 행을 한 번에** 맞바꾸고, 서비스를 거쳐 `ui → core → infra`를 지켰다. 아이콘 세트를 **6종 → 8종**으로 늘렸다(↑↓, 툴팁 필수). **화면 확인에서 세 번 되돌아갔다** — ① 목록을 520으로 줄이며 편집 카드를 우측 끝에 둬 가운데가 750px 비었다(둘을 왼쪽에 붙임) ② 라벨의 `LABEL_VERT_OFFSET` 잔재로 에딧과 세로가 어긋났다 ③ **단일 행 에딧은 `EM_SETRECT`가 안 먹어** 여백으로 못 밀고, 높이를 `TEXTMETRIC::tmHeight`로 **실측**해야 세로 가운데가 된다(20으로 추측했다가 틀렸다). 그 밖에 **헤더 fmt가 셀 정렬까지 바꾸고 있던 것**(`HDF_CENTER` 잔재로 법인명이 가운데였다)과 **떨어져 있던 검색 라벨**을 걷어냈고, `CSageSearchBox`를 신설해 입력+🔍를 한 테두리로 묶었다(검색 아이콘 그리기는 `SageUiStyle`로 올려 버튼과 공유). **작업 중 실수 2건**: 브랜치를 만들지 않고 **`develop`에 직접 커밋**해 브랜치로 옮기고 되돌렸다. `SageUiStyle`로 아이콘을 옮기며 **include를 빠뜨려** 빌드가 깨졌다
- **PR 링크**: 없음
- **결과**: 빌드·화면 확인 완료(2단 배치 · 정렬 · 검색 박스 · 편집 흐름 · ↑↓). 머지 대기

## [2026-08-07] feature/history-table
- **목적**: D7-5 — 실행 기록을 텍스트 로그에서 표로 바꾸고 필터 pill을 붙인다
- **변경 내용**: 착수 전 목업·스킬을 다시 재면서 **계획서가 틀린 것 하나**를 찾았다 — D7-5 명세가 요약을 `CSageSummaryBar`로 적었는데 그건 3-1의 「라벨+수치+단위」 텍스트 세그먼트이고, **3-5의 셋은 radius 999 pill이며 하나가 선택된 상태**다(스킬 팔레트도 `#F7F2EA`를 「필터 pill」로 규정). 다른 컨트롤이라 신설했다. 사용자가 **「동작 열」과 「기록 내보내기」를 뺐고**, 처음엔 그 결과를 목록에서 줄만 그어 보고했다가 **「UI가 조금은 달라지잖아」**는 지적을 받고 화면을 다시 구성했다 — 다만 그 과정에서 D7-5 본체 작업(표 전환·빈 상태)과 내 취향(선택 강조 끄기·툴팁)을 컷의 결과인 것처럼 섞어 놓아 **간단한 변경을 복잡하게 보이게 만들었고**, 두 번째 지적을 받고 걷어냈다. **행 높이는 34로 갔다** — 목업 3-5만 40이고 3-1은 34인데 스킬이 「모든 표가 규격을 공유한다」로 못박고 있다. 그래서 실패 원인을 셀 안 2줄로 넣을 수 없어 **「사유」 열**로 뺐다(사용자 결정, 스킬의 결과 테이블 기본 컬럼이 이미 「항목·값·상태·사유」다). 컬럼 폭은 GDI 실측 비율(**296:360:208**)로 나눴다 — 균등이면 저장 경로만 잘린다. **착수해 보니 셋이 드러났다**: ① 폭 배분 로직이 **stretch 열 하나만 전제**해 목업의 `1fr 1fr`만으로도 표가 폭을 넘긴다 → `SageWorkflowResultTable::DistributeColumnWidths`(core)로 뽑아 비율 배분으로 고쳤고 결과 표도 공유한다(stretch 1개면 결과 동일) ② **목업에 없는 섹션 밴드**가 화면에 있었다(D7-4가 미뤄둔 자리) → 지우니 `GetDetailSectionLabel()` 사용처가 0이 되어 **인터페이스 1 + 구현 3 + 상수 2**를 함께 걷어냈고 **워크플로 추가 비용이 한 메서드 줄었다** ③ **빈 상태가 없었다** → 기록 0건과 **필터 결과 0건**(「실패 0」 클릭)을 다른 문구로 갈랐다. `CSageListCtrl`에는 배지 열과 행 상태 스타일을 넣되 **컨트롤이 「성공/실패」를 모르게** `SetItemData`의 상태 번호로만 갈랐다. **작업 중 실수 1건**: python heredoc으로 상수를 지우려다 **한글이 깨져 치환은 실패했는데 파일은 재작성되어 CRLF 939줄이 LF가 됐다** — `sed -i` 2회에 이어 **세 번째 인코딩 사고**라 부채로 등록했다
- **PR 링크**: 없음
- **결과**: 빌드·화면 확인 완료(빈 상태 · 배지 · 실패 행 · 필터 pill · 결과 표 폭). 머지 대기

## [2026-08-07] feature/status-card-actions
- **목적**: D7-4 3단계 — 완료 카드에 후속 동작 버튼을 붙이고 D7-4를 닫는다
- **변경 내용**: 목업 3-4의 후속 버튼은 `height:30`인데 **스킬은 「컨트롤 높이 32 단일」**이라 어긋났다. 1단계에서 34를 신설한 전례가 있어 30을 또 만들면 한 화면에 32/34/30 세 높이가 되므로 물었고, **32로 결정받았다.** 폭은 88로 추측했다가 **스킬이 규정한 GDI 실측**(`AddFontResourceEx(FR_PRIVATE)` + `GetTextExtentPoint32W`, `GetTextFaceW`로 Pretendard 확인)을 돌려 **80**(텍스트 52 + 패딩 28)으로 고쳤다. 버튼 소유는 `CSageEmptyState` 방식 그대로 — **컨트롤이 버튼을 갖고 클릭만 부모가 준 command id로 올린다.** 「결과 보기」는 만들었다가 **사용자 결정으로 걷어냈다**(ID 2개 · `WM_` 1개 · 핸들러 2개 · `HasResultTab()` · `SetResult` 4번째 인자까지 함께 제거). 화면을 본 뒤 **1단계로 되돌아간 수정 2건**: 입력 패널만 에딧 좌측 여백이 0이어서 10으로(목업 `padding:0 10px`), 폼 라벨 폭 80이 실측 52보다 28 넓어 라벨이 입력과 40px 떨어져 보이던 것을 **64**로 내렸다 — 로그인·단가 폼과 같은 값이라 **앱의 모든 폼 라벨 폭이 하나로 모였고** 상수에서 `_WIDE`가 떨어졌다. **작업 중 실수 1건**: `sed -i`로 호출부를 일괄 치환하다 **새로 만든 `ClearStatusCard()` 본문까지 바꿔 무한 재귀**를 만들었고 CRLF도 함께 깨뜨렸다 — 4d-3 교훈(「일괄 치환에 검증을 붙인다」)을 또 어겼다
- **덧붙인 수정**: 사용자가 **「납품서에서 행을 클릭해도 생성이 활성화되지 않는다」**를 발견했다. **또 4d-3 회귀**였다 — View의 `OnResultTableChanged` · `OnResultSelectionChanged`가 4d-3에서 삭제됐는데 **패널들의 `ForwardToParent`는 그대로 남아** 선택 통지가 빈 곳으로 갔다. `ApplyActionButtonState`가 한 번도 불리지 않아 `UsesInputTable()`인 납품·견적만 계속 비활성이었다(미수금은 선택과 무관해 증상이 안 보였다). **View로 되돌리지 않고 워크스페이스가 직접 소비하게 했다** — 결과 요약·버튼 상태는 워크스페이스의 일이고 `coding-design`의 「패널이 자기 메시지맵에서 받아 갱신한다」에 맞는다. 이어서 **커스텀 메시지 10종을 전수 대조**해 남은 끊김이 없음을 확인했다
- **PR 링크**: 없음
- **결과**: 빌드·화면 확인 완료(폴더 열기 · 에딧 여백 · 라벨 폭 · 행 선택 활성화). 머지 대기
