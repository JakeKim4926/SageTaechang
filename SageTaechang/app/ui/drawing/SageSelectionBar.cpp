...
BEGIN_MESSAGE_MAP(CSageSelectionBar, CStatic)
	...
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(ID_SAGE_SELECTION_CHECK, &CSageSelectionBar::OnSelectAllClicked)
	ON_BN_CLICKED(ID_SAGE_SELECTION_CLEAR, &CSageSelectionBar::OnClearClicked)
END_MESSAGE_MAP()
...
int CSageSelectionBar::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	...
	m_wndSelectAll.Create(SAGE_UI_SELECT_ALL_BUTTON, WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
		rectEmpty, this, ID_SAGE_SELECTION_CHECK);
	m_wndSelectAll.SetFont(SageUiResources::GetFont(SAGE_FONT_CONTENT));

	m_wndClearBtn.Create(SAGE_UI_SELECTION_CLEAR_BUTTON, WS_CHILD | WS_VISIBLE | BS_OWNERDRAW,
		rectEmpty, this, ID_SAGE_SELECTION_CLEAR);
...
int CSageSelectionBar::GetCheckWidth(CDC* pDC) const {
	return SAGE_SELECTION_CHECK_GLYPH_WIDTH
		+ MeasureTextWidth(pDC, SAGE_UI_SELECT_ALL_BUTTON, SAGE_FONT_CONTENT);
}

int CSageSelectionBar::GetCountWidth(CDC* pDC) const {
	CString strTotal;
	strTotal.Format(SAGE_UI_SELECTION_TOTAL_FORMAT, m_nTotalCount);
	CString strSelected;
	strSelected.Format(SAGE_UI_SELECTION_SELECTED_FORMAT, m_nSelectedCount);

	return MeasureTextWidth(pDC, strTotal, SAGE_FONT_CONTENT)
		+ SAGE_ICON_TEXT_GAP + MeasureTextWidth(pDC, strSelected, SAGE_FONT_CONTENT_SEMIBOLD)
		+ SAGE_ICON_TEXT_GAP + MeasureTextWidth(pDC, SAGE_UI_SELECTION_SUFFIX, SAGE_FONT_CONTENT);
}

int CSageSelectionBar::GetClearWidth(CDC* pDC) const {
	return MeasureTextWidth(pDC, SAGE_UI_SELECTION_CLEAR_BUTTON, SAGE_FONT_CONTENT)
...
int CSageSelectionBar::GetContentWidth() const {
	...
	return GetCheckWidth(&dc) + SAGE_SELECTION_BAR_GAP + GetCountWidth(&dc)
		+ SAGE_SELECTION_BAR_GAP + GetClearWidth(&dc);
}
...
void CSageSelectionBar::LayoutChildren() {
	...
	int nClearLeft = nCheckWidth + SAGE_SELECTION_BAR_GAP + GetCountWidth(&dc) + SAGE_SELECTION_BAR_GAP;
...
void CSageSelectionBar::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
	...
	int nLeft = GetCheckWidth(pDC) + SAGE_SELECTION_BAR_GAP;
	nLeft = DrawTextSegment(pDC, nLeft, rectClient, strTotal, SAGE_FONT_CONTENT, SAGE_COLOR_SECONDARY_TEXT);
	nLeft += SAGE_ICON_TEXT_GAP;
	nLeft = DrawTextSegment(pDC, nLeft, rectClient, strSelected, SAGE_FONT_CONTENT_SEMIBOLD, SAGE_COLOR_PRIMARY);
	nLeft += SAGE_ICON_TEXT_GAP;
	DrawTextSegment(pDC, nLeft, rectClient, SAGE_UI_SELECTION_SUFFIX, SAGE_FONT_CONTENT, SAGE_COLOR_SECONDARY_TEXT);
}
