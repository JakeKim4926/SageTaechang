...
LRESULT CSageHeaderCtrl::OnHeaderLayout(WPARAM wParam, LPARAM lParam) {
	...
	if (pLayout == NULL || pLayout->pwpos == NULL || pLayout->prc == NULL)
		return lResult;
	pLayout->pwpos->cy = SAGE_LIST_HEADER_HEIGHT;
	pLayout->prc->top = SAGE_LIST_HEADER_HEIGHT;
	return lResult;
}
...
void CSageHeaderCtrl::OnPaint() {
	...
	dc.SetTextColor(SAGE_COLOR_TEXT_MUTED);

	for (int i = 0; i < nCount; ++i) {
		CRect rcItem;
		GetItemRect(i, &rcItem);

		dc.FillSolidRect(rcItem, SAGE_COLOR_LIST_HEADER);
