...
BEGIN_MESSAGE_MAP(SageWorkspacePanel, CWnd)
	ON_WM_CREATE()
	ON_WM_ERASEBKGND()
	ON_NOTIFY(TCN_SELCHANGE, ID_SAGE_TASK_TABS, &SageWorkspacePanel::OnTabChanged)
	ON_MESSAGE(WM_SAGE_RESULT_TABLE_CHANGED, &SageWorkspacePanel::OnResultTableChanged)
	ON_MESSAGE(WM_SAGE_RESULT_SELECTION_CHANGED, &SageWorkspacePanel::OnResultSelectionChanged)
	ON_MESSAGE(WM_SAGE_WORKFLOW_RUN_REQUESTED, &SageWorkspacePanel::OnWorkflowRunRequested)
	ON_MESSAGE(WM_SAGE_WORKFLOW_INPUT_RESET, &SageWorkspacePanel::OnWorkflowInputReset)
	ON_MESSAGE(WM_SAGE_OPEN_OUTPUT_FOLDER, &SageWorkspacePanel::OnOpenOutputFolder)
	ON_MESSAGE(WM_SAGE_WORKFLOW_COMPLETE, &SageWorkspacePanel::OnWorkflowComplete)
END_MESSAGE_MAP()

SageWorkspacePanel::SageWorkspacePanel()
	: m_pHandler(NULL)
	, m_nCurrentWorkflow(SAGE_WORKFLOW_DELIVERY)
	, m_nSelectedTaskTab(SAGE_TAB_INDEX_INPUT) {
}
...
int SageWorkspacePanel::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	...
	CRect r(0, 0, 0, 0);
	m_wndTaskTabs.Create(WS_CHILD | WS_VISIBLE | TCS_FIXEDWIDTH, r, this, ID_SAGE_TASK_TABS);
	...
	m_panelWorkflowInput.Create(this, ID_SAGE_WORKFLOW_INPUT_PANEL);
	m_panelWorkflowResult.Create(this, ID_SAGE_WORKFLOW_RESULT_PANEL);
	m_panelWorkflowHistory.Create(this, ID_SAGE_WORKFLOW_HISTORY_PANEL);
	m_panelCompanyOrder.Create(this, ID_SAGE_COMPANY_ORDER_PANEL);
	return 0;
}
...
BOOL SageWorkspacePanel::IsInputTabSelected() const {
	return (m_nSelectedTaskTab == SAGE_TAB_INDEX_INPUT) ? TRUE : FALSE;
}

BOOL SageWorkspacePanel::IsResultTab() const {
	return (m_nSelectedTaskTab == SAGE_TAB_INDEX_DOCUMENT_RESULT) ? TRUE : FALSE;
}

BOOL SageWorkspacePanel::IsDetailTab() const {
	return (m_nSelectedTaskTab == SAGE_TAB_INDEX_DOCUMENT_HISTORY) ? TRUE : FALSE;
}

BOOL SageWorkspacePanel::IsDataManageTab() const {
	return (m_nCurrentWorkflow == SAGE_WORKFLOW_RECEIVABLES &&
		m_nSelectedTaskTab == SAGE_TAB_INDEX_DOCUMENT_DATA_MANAGE) ? TRUE : FALSE;
}

int SageWorkspacePanel::GetTabVisualIndex(int nSemanticTabIndex) const {
	if (m_pHandler == NULL)
		return SAGE_TAB_INDEX_INPUT;
...
CRect SageWorkspacePanel::GetContentRect() const {
	...
	int nTop = IsPriceWorkflow() ? 0 : SAGE_TAB_HEIGHT;
	return CRect(
		SAGE_CONTENT_PAD_X,
		nTop + SAGE_CONTENT_PAD_Y,
		rectClient.Width() - SAGE_CONTENT_PAD_X,
		rectClient.Height() - SAGE_CONTENT_PAD_Y);
}
...
void SageWorkspacePanel::LayoutTabRow() {
	...
	m_wndTaskTabs.MoveWindow(
		SAGE_CONTENT_PAD_X,
		0,
		rectClient.Width() - SAGE_CONTENT_PAD_X * 2,
		SAGE_TAB_HEIGHT);
}
...
void SageWorkspacePanel::LayoutActivePanel() {
	...
	if (m_nCurrentWorkflow == SAGE_WORKFLOW_PRICE_CALC) {
	...
	if (IsInputTabSelected()) {
		m_panelWorkflowInput.Layout(CRect(
			rectContent.left,
			rectContent.top,
			rectContent.right + SAGE_EDIT_BORDER_WIDTH,
			rectContent.bottom));
		return;
	}

	int nBodyHeight = max(SAGE_RESULT_MIN_HEIGHT, rectContent.Height() - SAGE_RESULT_HEADER_HEIGHT);
	if (IsResultTab()) {
		m_panelWorkflowResult.Layout(CRect(
			rectContent.left,
			rectContent.top - m_panelWorkflowResult.GetBandHeight(),
			rectContent.right,
			rectContent.top + SAGE_RESULT_HEADER_HEIGHT + nBodyHeight));
		return;
	}
	if (IsDetailTab()) {
...
void SageWorkspacePanel::UpdateVisibility(const SageWorkspaceVisibility& state) {
	...
	m_panelPriceManage.ShowWindow((m_nCurrentWorkflow == SAGE_WORKFLOW_PRICE_MANAGE) ? SW_SHOW : SW_HIDE);
	m_panelPriceCalc.ShowWindow((m_nCurrentWorkflow == SAGE_WORKFLOW_PRICE_CALC) ? SW_SHOW : SW_HIDE);
}
...
BOOL SageWorkspacePanel::OnEraseBkgnd(CDC* pDC) {
	...
	pDC->FillSolidRect(0, 0, rectClient.Width(), SAGE_TAB_HEIGHT, SAGE_COLOR_PANEL);
	pDC->FillSolidRect(
		0,
		SAGE_TAB_HEIGHT - SAGE_BORDER_THICKNESS,
		rectClient.Width(),
...
void SageWorkspacePanel::OnTabChanged(NMHDR* pNMHDR, LRESULT* pResult) {
	...
	*pResult = 0;
	if (IsDataManageTab())
		m_panelCompanyOrder.RefreshList();
	ForwardToParent(WM_SAGE_WORKSPACE_TAB_CHANGED, 0, 0);
}
...
LRESULT SageWorkspacePanel::OnResultTableChanged(WPARAM wParam, LPARAM lParam) {
	...
	ForwardToParent(WM_SAGE_WORKSPACE_STATE_CHANGED, 0, 0);
	return 0;
}
...
void SageWorkspacePanel::SetResultTableRows(const std::vector<SageResultRow>& arrRows) {
	SageResultTablePanel* pPanel = FindResultTable();
	if (pPanel == NULL)
		return;
...
BOOL SageWorkspacePanel::ValidateInputPath(CString& strInputPath) const {
	...
	if (!strInputPath.IsEmpty())
		return TRUE;
	ShowSageMessageBox(SAGE_UI_INPUT_REQUIRED, MB_ICONWARNING);
	return FALSE;
}
...
BOOL SageWorkspacePanel::ValidateOutputFolder(CString& strOutputFolder) const {
	...
	if (!strOutputFolder.IsEmpty())
		return TRUE;
	ShowSageMessageBox(SAGE_UI_OUTPUT_REQUIRED, MB_ICONWARNING);
	return FALSE;
}
...
BOOL SageWorkspacePanel::BuildSelectedRowNums(int nTaskType, CString& strRowNums, BOOL& bOnePage) {
	...
	strRowNums.Empty();
	if (pPanel == NULL || !m_pHandler->UsesInputTable() || nTaskType != SAGE_TASK_GENERATE)
		return TRUE;
	...
	for (int i = 0; i < nRowCount; ++i) {
		...
		if (!strRowNums.IsEmpty())
			strRowNums += SAGE_UI_ROW_NUM_SEPARATOR;
		strRowNums += strNum;
	}
...
void SageWorkspacePanel::RequestRun(int nTaskType) {
	...
	if (!ValidateInputPath(strInputPath))
		return;
	if (nTaskType == SAGE_TASK_GENERATE && !ValidateOutputFolder(strOutputFolder))
		return;
...
void SageWorkspacePanel::ApplyDroppedInputPaths(const CString& strPaths) {
	...
	int nIndex = 0;
	CString strInputPaths = strPaths.Tokenize(SAGE_UI_DROP_PATH_SEPARATOR, nIndex);
	strInputPaths.Trim();
	...
	if (!IsInputTabSelected()) {
		SelectTab(SAGE_TAB_INDEX_INPUT);
		RefreshVisibility();
		LayoutActivePanel();
	}
	NotifyStatus(SAGE_UI_DROP_RECEIVED);
...
void SageWorkspacePanel::DisplayResponse(int nWorkflowType, int nTaskType, const CString& strResponseJson) {
	...
	BOOL bKeepInputTable =
		(nTaskType == SAGE_TASK_GENERATE && pHandler != NULL && pHandler->UsesInputTable())
		? TRUE : FALSE;

	SageWorkflowResultPresenter presenter;
	std::vector<SageResultRow> arrRows;
	...
	if (pHandler != NULL && (nTaskType == SAGE_TASK_LOAD || nTaskType == SAGE_TASK_GENERATE)) {
		SelectTab(pHandler->UsesInputTable()
			? SAGE_TAB_INDEX_INPUT
			: SAGE_TAB_INDEX_DOCUMENT_RESULT);
		RefreshVisibility();
		LayoutActivePanel();
		if (nTaskType == SAGE_TASK_GENERATE && bSuccess) {
	...
	NotifyStatus(bSuccess ? SAGE_UI_COMPLETED : SAGE_UI_FAILED);
	ForwardToParent(WM_SAGE_WORKSPACE_STATE_CHANGED, 0, 0);
}
...
void SageWorkspacePanel::ApplyStatusCardResult(
	...
	if (!bSuccess) {
		if (nTaskType == SAGE_TASK_LOAD)
			strMessage = SAGE_UI_STATUS_CARD_LOAD_FAILED;
		else
			strMessage.Format(SAGE_UI_STATUS_CARD_FAILED_FORMAT, pHandler->GetActionButtonLabel());
		strDetail = JsonExtractString(strResponseJson, SAGE_JSON_KEY_MESSAGE);
		if (strDetail.IsEmpty())
			strDetail = JsonExtractString(strResponseJson, SAGE_JSON_KEY_CODE);
		m_panelWorkflowInput.SetStatusResult(FALSE, strMessage, strDetail);
		return;
	}

	if (nTaskType == SAGE_TASK_LOAD)
		strMessage.Format(SAGE_UI_STATUS_CARD_LOAD_COMPLETED_FORMAT, nResultCount);
	else
		strMessage.Format(SAGE_UI_STATUS_CARD_COMPLETED_FORMAT, pHandler->GetActionButtonLabel(), nResultCount);

	strDetail = JsonExtractString(strResponseJson, SAGE_JSON_KEY_FILE_PATH);
	if (strDetail.IsEmpty())
		strDetail = JsonExtractString(strResponseJson, SAGE_JSON_KEY_OUTPUT_FOLDER);
...
LRESULT SageWorkspacePanel::OnOpenOutputFolder(WPARAM wParam, LPARAM lParam) {
	...
	if (dwAttributes == INVALID_FILE_ATTRIBUTES) {
		ShowSageMessageBox(SAGE_UI_OUTPUT_PATH_MISSING, MB_ICONWARNING);
		return 0;
	}

	if ((dwAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0) {
		::ShellExecuteW(GetSafeHwnd(), SAGE_UI_EXPLORER_VERB_OPEN,
			m_strLastOutputPath, NULL, NULL, SW_SHOWNORMAL);
		return 0;
	}

	CString strArguments;
	strArguments.Format(SAGE_UI_EXPLORER_SELECT_FORMAT, static_cast<LPCWSTR>(m_strLastOutputPath));
	::ShellExecuteW(GetSafeHwnd(), NULL, SAGE_UI_EXPLORER_COMMAND,
...
void SageWorkspacePanel::NotifyStatus(LPCWSTR pszStatus) {
	...
	if (pParent == NULL || !::IsWindow(pParent->GetSafeHwnd()))
		return;
	pParent->SendMessage(WM_SAGE_WORKSPACE_STATUS, 0, reinterpret_cast<LPARAM>(pszStatus));
}

SageWorkflowUiState& SageWorkspacePanel::GetWorkflowState(int nWorkflowType) {
	if (nWorkflowType == SAGE_WORKFLOW_DELIVERY)
		return m_stateDelivery;
	if (nWorkflowType == SAGE_WORKFLOW_ESTIMATE)
