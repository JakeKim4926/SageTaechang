...
#include "app/common/SageDialogHelper.h"
...
namespace
{
	constexpr const wchar_t* SAGE_RECEIVABLES_LOAD_SCRIPT_PATH = L"tools\\load-receivables-data.ps1";
	constexpr const wchar_t* SAGE_RECEIVABLES_GEN_SCRIPT_PATH = L"tools\\generate-receivables-form.ps1";
	constexpr const wchar_t* SAGE_RECEIVABLES_TEMPLATE_PATH = L"templates\\receivables-template.xls";
	constexpr const wchar_t* SAGE_RECEIVABLES_ERR_INPUT_REQUIRED = L"SNX_SAGE_RECEIVABLES_001";
	constexpr const wchar_t* SAGE_RECEIVABLES_ERR_OUTPUT_REQUIRED = L"SNX_SAGE_RECEIVABLES_002";
	constexpr const wchar_t* SAGE_RECEIVABLES_ERR_TEMPLATE_MISSING = L"SNX_SAGE_RECEIVABLES_003";
	constexpr const wchar_t* SAGE_RECEIVABLES_ERR_SCRIPT_MISSING = L"SNX_SAGE_RECEIVABLES_004";
	constexpr const wchar_t* SAGE_RECEIVABLES_ERR_PROCESS_FAILED = L"SNX_SAGE_RECEIVABLES_005";
	constexpr const wchar_t* SAGE_RECEIVABLES_ERR_RESULT_MISSING = L"SNX_SAGE_RECEIVABLES_006";
	...
	CString BuildReceivableCompanyOrderJson(
		const CArray<SageReceivableCompanyOrderDto, SageReceivableCompanyOrderDto&>& arrOrder) {
		CString strJson;

		strJson = L"[";
		for (INT_PTR nIndex = 0; nIndex < arrOrder.GetCount(); ++nIndex) {
			const SageReceivableCompanyOrderDto& dto = arrOrder.GetAt(nIndex);
	...
	BOOL BuildReceivableCompanyOrderFile(CString& outPriorityPath, CString& strError) {
		...
		SageReceivableCompanyOrderService* pService;
	...
	CString HandleLoadInputData(
		...
		if (strInputPath.IsEmpty() || !FileExists(strInputPath))
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_INPUT_REQUIRED, L"파일을 선택해주세요.");

		CString strPluginDirectory;
		if (!GetExecutableDirectory(strPluginDirectory))
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_SCRIPT_MISSING, L"플러그인 경로를 확인하지 못했습니다.");

		CString strScriptPath = CombinePath(strPluginDirectory, SAGE_RECEIVABLES_LOAD_SCRIPT_PATH);
		if (!FileExists(strScriptPath))
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_SCRIPT_MISSING, L"미수금 데이터 로드 스크립트가 없습니다.");
		...
		if (BuildReceivableCompanyOrderFile(strPriorityPath, strError) == FALSE) {
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_PROCESS_FAILED, strError);
		}
		...
		if (!ReadUtf8File(strResultPath, strResultJson, strError)) {
			if (strPriorityPath.IsEmpty() == FALSE)
				DeleteFileW(strPriorityPath);
			DeleteFileW(strResultPath);
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_RESULT_MISSING, strError);
		}
		if (strPriorityPath.IsEmpty() == FALSE)
			DeleteFileW(strPriorityPath);
		DeleteFileW(strResultPath);
		...
		if (dwExitCode != 0) {
			...
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_PROCESS_FAILED, strMessage);
		}
	...
	CString HandleSelectOutputFolder(const CString& strRequestId) {
		...
		if (FAILED(hr) || pDialog == NULL) {
			if (bNeedUninitialize)
				CoUninitialize();
			return BuildErrorResponse(strRequestId, L"SNX_SAGE_RECEIVABLES_007", L"폴더 선택 창을 열지 못했습니다.");
		}
	...
	CString HandleGenerateForm(
		...
		if (strOutputFolder.IsEmpty() || !FolderExists(strOutputFolder))
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_OUTPUT_REQUIRED, L"저장 폴더를 선택해주세요.");

		CString strPluginDirectory;
		if (!GetExecutableDirectory(strPluginDirectory))
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_SCRIPT_MISSING, L"플러그인 경로를 확인하지 못했습니다.");

		CString strTemplatePath = CombinePath(strPluginDirectory, SAGE_RECEIVABLES_TEMPLATE_PATH);
		CString strScriptPath = CombinePath(strPluginDirectory, SAGE_RECEIVABLES_GEN_SCRIPT_PATH);

		if (!FileExists(strTemplatePath))
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_TEMPLATE_MISSING, L"미수금 내역서 템플릿 파일이 없습니다.");

		if (!FileExists(strScriptPath))
			return BuildErrorResponse(strRequestId, SAGE_RECEIVABLES_ERR_SCRIPT_MISSING, L"미수금 내역서 생성 스크립트가 없습니다.");
...
CString SageReceivablesExcelService::BuildOpenInputDialogResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
...
CString SageReceivablesExcelService::BuildLoadInputDataResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
	return HandleLoadInputData(strRequestId, strPayloadJson);
}

CString SageReceivablesExcelService::BuildSelectOutputFolderResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
	UNREFERENCED_PARAMETER(strPayloadJson);
...
CString SageReceivablesExcelService::BuildGenerateResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
