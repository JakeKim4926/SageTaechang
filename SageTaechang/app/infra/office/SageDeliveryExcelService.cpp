...
namespace
{
	constexpr const wchar_t* SAGE_DELIVERY_LOAD_SCRIPT_PATH = L"tools\\load-delivery-data.ps1";
	constexpr const wchar_t* SAGE_DELIVERY_SCRIPT_PATH = L"tools\\generate-delivery-forms.ps1";
	constexpr const wchar_t* SAGE_DELIVERY_TEMPLATE_PATH = L"templates\\delivery-template.xls";
	constexpr const wchar_t* SAGE_ERROR_INPUT_REQUIRED = L"SNX_SAGE_DELIVERY_001";
	constexpr const wchar_t* SAGE_ERROR_OUTPUT_REQUIRED = L"SNX_SAGE_DELIVERY_002";
	constexpr const wchar_t* SAGE_ERROR_TEMPLATE_MISSING = L"SNX_SAGE_DELIVERY_003";
	constexpr const wchar_t* SAGE_ERROR_SCRIPT_MISSING = L"SNX_SAGE_DELIVERY_004";
	constexpr const wchar_t* SAGE_ERROR_PROCESS_FAILED = L"SNX_SAGE_DELIVERY_005";
	constexpr const wchar_t* SAGE_ERROR_RESULT_MISSING = L"SNX_SAGE_DELIVERY_006";
	constexpr const wchar_t* SAGE_ERROR_NO_SELECTION = L"SNX_SAGE_DELIVERY_008";
	...
	CString HandleLoadInputDataResponse(
		...
		if (strInputPath.IsEmpty() || !FileExists(strInputPath))
			return BuildErrorResponse(strRequestId, SAGE_ERROR_INPUT_REQUIRED, L"?뚯씪???좏깮?댁＜?몄슂.");

		CString strPluginDirectory;
		if (!GetExecutableDirectory(strPluginDirectory))
			return BuildErrorResponse(strRequestId, SAGE_ERROR_SCRIPT_MISSING, L"?뚮윭洹몄씤 寃쎈줈瑜??뺤씤?섏? 紐삵뻽?듬땲??");

		CString strScriptPath = CombinePath(strPluginDirectory, SAGE_DELIVERY_LOAD_SCRIPT_PATH);
		if (!FileExists(strScriptPath))
			return BuildErrorResponse(strRequestId, SAGE_ERROR_SCRIPT_MISSING, L"?곗씠??濡쒕뱶 ?ㅽ겕由쏀듃媛 ?놁뒿?덈떎.");
		...
		if (!RunProcessAndWait(strCommandLine, dwExitCode, strError)) {
			...
			return BuildErrorResponse(strRequestId, SAGE_ERROR_PROCESS_FAILED, strError);
		}

		CString strResultJson;
		if (!ReadUtf8File(strResultPath, strResultJson, strError)) {
			DeleteFileW(strResultPath);
			return BuildErrorResponse(strRequestId, SAGE_ERROR_RESULT_MISSING, strError);
		}
		DeleteFileW(strResultPath);
		...
		if (dwExitCode != 0) {
			...
			return BuildErrorResponse(strRequestId, SAGE_ERROR_PROCESS_FAILED, strMessage);
		}
	...
	CString HandleSelectOutputFolderResponse(const CString& strRequestId) {
		...
		if (FAILED(hr) || pDialog == NULL) {
			if (bNeedUninitialize)
				CoUninitialize();
			return BuildErrorResponse(strRequestId, L"SNX_SAGE_DELIVERY_007", L"?대뜑 ?좏깮 李쎌쓣 ?댁? 紐삵뻽?듬땲??");
		}
	...
	CString HandleGenerateResponse(
		...
		if (strOutputFolder.IsEmpty() || !FolderExists(strOutputFolder))
			return BuildErrorResponse(strRequestId, SAGE_ERROR_OUTPUT_REQUIRED, L"????대뜑瑜??좏깮?댁＜?몄슂.");

		CString strPluginDirectory;
		if (!GetExecutableDirectory(strPluginDirectory))
			return BuildErrorResponse(strRequestId, SAGE_ERROR_SCRIPT_MISSING, L"?뚮윭洹몄씤 寃쎈줈瑜??뺤씤?섏? 紐삵뻽?듬땲??");

		CString strTemplatePath = CombinePath(strPluginDirectory, SAGE_DELIVERY_TEMPLATE_PATH);
		CString strScriptPath = CombinePath(strPluginDirectory, SAGE_DELIVERY_SCRIPT_PATH);

		if (!FileExists(strTemplatePath))
			return BuildErrorResponse(strRequestId, SAGE_ERROR_TEMPLATE_MISSING, L"?⑺뭹???쒗뵆由??뚯씪???놁뒿?덈떎.");

		if (!FileExists(strScriptPath))
			return BuildErrorResponse(strRequestId, SAGE_ERROR_SCRIPT_MISSING, L"?⑺뭹???앹꽦 ?ㅽ겕由쏀듃媛 ?놁뒿?덈떎.");
...
CString SageDeliveryExcelService::BuildOpenInputDialogResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
...
CString SageDeliveryExcelService::BuildLoadInputDataResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
	return HandleLoadInputDataResponse(strRequestId, strPayloadJson);
}

CString SageDeliveryExcelService::BuildSelectOutputFolderResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
	UNREFERENCED_PARAMETER(strPayloadJson);
...
CString SageDeliveryExcelService::BuildGenerateResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
