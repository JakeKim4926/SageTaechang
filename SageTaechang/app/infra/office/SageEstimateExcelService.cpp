...
namespace
{
	constexpr const wchar_t* SAGE_ESTIMATE_LOAD_SCRIPT_PATH = L"tools\\load-input-data.ps1";
	constexpr const wchar_t* SAGE_ESTIMATE_GEN_SCRIPT_PATH = L"tools\\generate-estimate-forms.ps1";
	constexpr const wchar_t* SAGE_ESTIMATE_TEMPLATE_PATH = L"templates\\estimate-template.xlsx";
	constexpr const wchar_t* SAGE_ESTIMATE_ERR_INPUT_REQUIRED = L"SNX_SAGE_ESTIMATE_001";
	constexpr const wchar_t* SAGE_ESTIMATE_ERR_OUTPUT_REQUIRED = L"SNX_SAGE_ESTIMATE_002";
	constexpr const wchar_t* SAGE_ESTIMATE_ERR_TEMPLATE_MISSING = L"SNX_SAGE_ESTIMATE_003";
	constexpr const wchar_t* SAGE_ESTIMATE_ERR_SCRIPT_MISSING = L"SNX_SAGE_ESTIMATE_004";
	constexpr const wchar_t* SAGE_ESTIMATE_ERR_PROCESS_FAILED = L"SNX_SAGE_ESTIMATE_005";
	constexpr const wchar_t* SAGE_ESTIMATE_ERR_RESULT_MISSING = L"SNX_SAGE_ESTIMATE_006";
	constexpr const wchar_t* SAGE_ESTIMATE_ERR_NO_SELECTION = L"SNX_SAGE_ESTIMATE_007";
	...
	CString HandleLoadInputData(
		...
		if (strInputPath.IsEmpty() || !FileExists(strInputPath))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_INPUT_REQUIRED, L"?뚯씪???좏깮?댁＜?몄슂.");

		CString strPluginDirectory;
		if (!GetExecutableDirectory(strPluginDirectory))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_SCRIPT_MISSING, L"?뚮윭洹몄씤 寃쎈줈瑜??뺤씤?섏? 紐삵뻽?듬땲??");

		CString strScriptPath = CombinePath(strPluginDirectory, SAGE_ESTIMATE_LOAD_SCRIPT_PATH);
		if (!FileExists(strScriptPath))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_SCRIPT_MISSING, L"?곗씠??濡쒕뱶 ?ㅽ겕由쏀듃媛 ?놁뒿?덈떎.");
		...
		if (!RunProcessAndWait(strCommandLine, dwExitCode, strError)) {
			...
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_PROCESS_FAILED, strError);
		}

		CString strResultJson;
		if (!ReadUtf8File(strResultPath, strResultJson, strError)) {
			DeleteFileW(strResultPath);
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_RESULT_MISSING, strError);
		}
		DeleteFileW(strResultPath);
		...
		if (dwExitCode != 0) {
			...
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_PROCESS_FAILED, strMessage);
		}
	...
	CString HandleSelectOutputFolder(const CString& strRequestId) {
		...
		if (FAILED(hr) || pDialog == NULL) {
			if (bNeedUninitialize)
				CoUninitialize();
			return BuildErrorResponse(strRequestId, L"SNX_SAGE_ESTIMATE_008", L"?대뜑 ?좏깮 李쎌쓣 ?댁? 紐삵뻽?듬땲??");
		}
	...
	CString HandleGenerateForms(
		...
		BOOL bEstimateOnePage = JsonExtractBool(strPayloadJson, SAGE_JSON_KEY_ESTIMATE_ONE_PAGE);

		if (strInputPath.IsEmpty() || !FileExists(strInputPath))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_INPUT_REQUIRED, L"?뚯씪???좏깮?댁＜?몄슂.");

		if (strOutputFolder.IsEmpty() || !FolderExists(strOutputFolder))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_OUTPUT_REQUIRED, L"????대뜑瑜??좏깮?댁＜?몄슂.");

		CString strPluginDirectory;
		if (!GetExecutableDirectory(strPluginDirectory))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_SCRIPT_MISSING, L"?뚮윭洹몄씤 寃쎈줈瑜??뺤씤?섏? 紐삵뻽?듬땲??");

		CString strTemplatePath = CombinePath(strPluginDirectory, SAGE_ESTIMATE_TEMPLATE_PATH);
		CString strScriptPath = CombinePath(strPluginDirectory, SAGE_ESTIMATE_GEN_SCRIPT_PATH);

		if (!FileExists(strTemplatePath))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_TEMPLATE_MISSING, L"寃ъ쟻???쒗뵆由??뚯씪???놁뒿?덈떎.");

		if (!FileExists(strScriptPath))
			return BuildErrorResponse(strRequestId, SAGE_ESTIMATE_ERR_SCRIPT_MISSING, L"寃ъ쟻???앹꽦 ?ㅽ겕由쏀듃媛 ?놁뒿?덈떎.");
...
CString SageEstimateExcelService::BuildOpenInputDialogResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
...
CString SageEstimateExcelService::BuildLoadInputDataResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
	return HandleLoadInputData(strRequestId, strPayloadJson);
}

CString SageEstimateExcelService::BuildSelectOutputFolderResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
	UNREFERENCED_PARAMETER(strPayloadJson);
...
CString SageEstimateExcelService::BuildGenerateResponse(
	const CString& strRequestId,
	const CString& strPayloadJson) {
