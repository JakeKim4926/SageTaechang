...
SagePriceService::SagePriceService(SagePriceRepository* pRepository) {
    m_pRepository = pRepository;
}

SagePriceService::~SagePriceService() {}

BOOL SagePriceService::AddPrice(const SagePriceDto& dto, int& nNewPriceId, CString& strError) {
    BOOL bExists;
    ...
    if (m_pRepository == NULL) {
        strError = _T("SagePriceRepository가 NULL입니다.");
        return FALSE;
    }
...
BOOL SagePriceService::LoadByCompany(
...
BOOL SagePriceService::LoadByCompanyAndCopies(
    const CString& strCompanyName,
    int nCopies,
    SagePriceDto& dto,
    BOOL& bFound,
...
BOOL SagePriceService::LoadByPrice(
    int nPrice,
    CArray<SagePriceDto, SagePriceDto&>& arrPrice,
    CString& strError
) {
...
BOOL SagePriceService::ChangePriceByCompany(
    const CString& strCompanyName,
    int nPrintPrice,
    int nCoverPrice,
    int& nAffectedCount,
...
BOOL SagePriceService::ChangePriceByCompanyAndCopies(
    const CString& strCompanyName,
    int nCopies,
    int nPrintPrice,
    int nCoverPrice,
...
BOOL SagePriceService::RenameCompany(
    ...
    if (m_pRepository == NULL) {
        strError = _T("SagePriceRepository媛 NULL?낅땲??");
        return FALSE;
    }
...
BOOL SagePriceService::ChangeCoverPriceByCompany(
    const CString& strCompanyName,
    int nCoverPrice,
    int& nAffectedCount,
    CString& strError
    ...
    if (nCoverPrice > SAGE_PRICE_AMOUNT_MAX) {
...
BOOL SagePriceService::RemovePrice(int nPriceId, CString& strError) {
    if (m_pRepository == NULL) {
        strError = _T("SagePriceRepository가 NULL입니다.");
        return FALSE;
    }
...
BOOL SagePriceService::RemoveCompany(
    const CString& strCompanyName,
    int& nAffectedCount,
    CString& strError
) {
    ...
    if (nAffectedCount <= 0) {
        strError = SAGE_UI_PRICE_DELETE_COMPANY_EMPTY;
        return FALSE;
    }
...
BOOL SagePriceService::AddCompany(
    const CString& strCompanyName,
    int& nNewCompanyId,
    CString& strError
) {
    ...
    if (m_pRepository == NULL) {
        strError = SAGE_PRICE_REPOSITORY_NULL_MESSAGE;
        return FALSE;
    }
...
BOOL SagePriceService::LoadAllCompanyNames(CStringArray& arrNames, CString& strError) {
    arrNames.RemoveAll();
...
BOOL SagePriceService::ModifyPriceById(const SagePriceDto& dto, CString& strError) {
    BOOL bExists;
...
BOOL SagePriceService::ValidateForInsert(const SagePriceDto& dto, CString& strError) {
...
BOOL SagePriceService::ValidateCompanyName(const CString& strCompanyName, CString& strError) {
...
BOOL SagePriceService::ValidateCopies(int nCopies, CString& strError) {
    if (nCopies < 1) {
...
BOOL SagePriceService::ValidatePriceValue(int nPrintPrice, int nCoverPrice, CString& strError) {
    if (nPrintPrice < 0) {
