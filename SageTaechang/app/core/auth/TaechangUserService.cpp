...
TaechangUserService::TaechangUserService(TaechangUserRepository* pRepository) {
    m_pRepository = pRepository;
}

TaechangUserService::~TaechangUserService() {}

BOOL TaechangUserService::Login(
...
BOOL TaechangUserService::AddUser(
    const TaechangUserDto& dto,
...
BOOL TaechangUserService::LoadAll(
...
BOOL TaechangUserService::RemoveUser(int nUserId, CString& strError) {
    return m_pRepository->Delete(nUserId, strError);
}

CString TaechangUserService::HashPassword(const CString& strPassword) {
...
BOOL TaechangUserService::ValidateLoginId(const CString& strLoginId, CString& strError) {
    if (strLoginId.IsEmpty()) {
        strError = _T("아이디를 입력하세요.");
        return FALSE;
    }

    if (strLoginId.GetLength() < TAECHANG_USER_LOGIN_ID_MIN_LEN) {
        strError.Format(_T("아이디는 %d자 이상이어야 합니다."), TAECHANG_USER_LOGIN_ID_MIN_LEN);
        return FALSE;
    }

    if (strLoginId.GetLength() > TAECHANG_USER_LOGIN_ID_MAX_LEN) {
        strError.Format(_T("아이디는 %d자 이하이어야 합니다."), TAECHANG_USER_LOGIN_ID_MAX_LEN);
        return FALSE;
    }
...
BOOL TaechangUserService::ValidatePassword(const CString& strPassword, CString& strError) {
